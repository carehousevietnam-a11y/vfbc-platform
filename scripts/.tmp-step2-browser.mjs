...
const out = { consoleErrors: [], interest: {}, noNotice: {} };
...
async function runFlow(page, noticeValue, tag) {
  const phase1 = {
  ...
  const noticeOpt =
    noticeValue === "interest_stated"
      ? "이자·가산금이 붙는다고"
      : "미납 시 어떻게 되는지";
  await clickChoice(page, noticeOpt);
  ...
  const details = await page.evaluate(() => {
    const el = document.querySelector('[data-purpose="ai-analysis-details"]');
    return el?.innerText ?? "";
  });
  return { details, simNotice: sim.case02_nonPaymentNotice };
}
...
try {
  ...
  const rI = await runFlow(pageI, "interest_stated", "int");
  out.interest = {
    simNotice: rI.simNotice,
    phase2InDom: rI.details.includes(PHASE2_INTEREST),
    phase2Snippet: rI.details.split("\n").filter((l) => l.includes("2차 추가") || l.includes("미납 시")),
  };
  await pageI.close();

  const pageN = await browser.newPage({ viewport: { width: 1280, height: 900 } });
  const rN = await runFlow(pageN, "no_notice", "nn");
  out.noNotice = {
    simNotice: rN.simNotice,
    itemInDom: rN.details.includes(NO_NOTICE_ITEM),
    section04Snippet: rN.details.split("\n").filter((l) => l.includes("확인") || l.includes("미납")),
  };
  await pageN.close();
} finally {
  await browser.close();
