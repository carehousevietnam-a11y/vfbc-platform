...
const VERIFY_QUESTION_CONTEXT = "불확실한 문서";
...
type UnclearAgency = (typeof UNCLEAR_AGENCY_OPTIONS)[number];
...
const UNCLEAR_AGENCY_GUIDANCE: Record<UnclearAgency, AgencyGuidance> = {
  "관공서 확인창구": {
...
const PREVENT_DOCUMENT_OPTIONS: ReviewQuestionOption[] = [
  { value: "정부기관서류", title: "정부기관 서류", desc: "정부·공공기관에서 발송된 것으로 보이는 서류" },
  { value: "법원서류", title: "법원 서류", desc: "법원에서 발송된 것으로 보이는 서류" },
  { value: "경찰서류", title: "경찰 서류", desc: "경찰(공안)에서 발송된 것으로 보이는 서류" },
  { value: "회사서류", title: "회사 서류", desc: "회사·기관 명의의 서류" },
  { value: "개인간서류", title: "개인 간 서류", desc: "개인 간 주고받은 서류" },
  { value: "출처불명", title: "출처불명 서류", desc: "발신처를 알 수 없는 서류" },
  { value: "기타", title: "기타", desc: "위 항목에 해당하지 않는 검토 자료" },
];

const CASE_ISSUE_OPTIONS: ReviewQuestionOption[] = [
  { value: "정부기관서류", title: "정부기관 서류 문제", desc: "정부기관 서류 대응 관련 문제" },
  { value: "법원서류", title: "법원 서류 문제", desc: "법원 서류 대응 관련 문제" },
  { value: "경찰서류", title: "경찰 서류 문제", desc: "경찰 서류 대응 관련 문제" },
  { value: "회사서류", title: "회사 서류 문제", desc: "회사 서류 관련 분쟁·문제" },
  { value: "개인간서류", title: "개인 간 분쟁", desc: "개인 간 서류로 인한 분쟁" },
  { value: "출처불명", title: "출처불명 서류 대응", desc: "출처를 알 수 없는 서류 대응 문제" },
...
function VerifyUnclearLeadCapture({
  riskLevel,
  messengers,
  lang,
  fieldErrors,
...
export default function VerifyUnclearPage() {
  ...
  const [selectedAgency, setSelectedAgency] = useState<UnclearAgency | null>(null);
  ...
  async function handleExpertRequest() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=verify_unclear&mode=expert`;
        return;
      }
  ...
  async function handleAiReportRequest() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=verify_unclear&mode=ai_report`;
        return;
      }
    ...
    >
        <FunnelPageHeader
          ...
          title={
            landingDone
              ? pageHeader.title
              : MASTER_LANDING_UNCLEAR.shortServiceLabel ?? MASTER_LANDING_UNCLEAR.serviceLabel
          }
        ...
        {landingDone && step === "diagnosis" && diagnosis && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <VerifyResultOverviewCards
              diagnosis={diagnosis}
              docCount={getRequiredDocuments("verify_unclear").documents.length}
            />
        ...
        {landingDone && step === "guidanceSelect" && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
              {UNCLEAR_AGENCY_OPTIONS.map((agency) => (
                <SelectionCard
                  key={agency}
                  title={agency}
