...
type FraudAgency = (typeof FRAUD_AGENCY_OPTIONS)[number];
...
const FRAUD_AGENCY_GUIDANCE: Record<FraudAgency, AgencyGuidance> = {
  "경찰 신고": {
...
const PREVENT_DOCUMENT_OPTIONS: ReviewQuestionOption[] = [
  { value: "투자사기", title: "투자 제안서", desc: "투자·수익 관련 제안서·계약서" },
  { value: "대출사기", title: "대출 제안서", desc: "대출·자금 조달 관련 서류" },
  { value: "온라인거래사기", title: "온라인 거래 내역", desc: "쇼핑몰·중고거래 등 온라인 거래 서류" },
  { value: "결혼·연애사기", title: "결혼·연애 관련 서류", desc: "결혼·연애 상대 관련 서류·대화" },
  { value: "사업제휴사기", title: "사업제휴 제안서", desc: "동업·제휴 관련 제안서·계약서" },
  { value: "기타", title: "기타", desc: "위 항목에 해당하지 않는 검토 자료" },
];

const CASE_ISSUE_OPTIONS: ReviewQuestionOption[] = [
  { value: "투자사기", title: "투자사기 피해", desc: "투자금 미회수·비정상 수익률 문제" },
  { value: "대출사기", title: "대출사기 피해", desc: "선입금 요구·대출 빙자 사기" },
  { value: "온라인거래사기", title: "온라인거래사기 피해", desc: "대금 지급 후 미배송·먹튀 문제" },
  { value: "결혼·연애사기", title: "결혼·연애사기 피해", desc: "금전 요구·신뢰 이용 피해" },
  { value: "사업제휴사기", title: "사업제휴사기 피해", desc: "동업금·투자금 편취 문제" },
...
function VerifyFraudLeadCapture({
  riskLevel,
  messengers,
  lang,
  fieldErrors,
...
export default function VerifyFraudPage() {
  ...
  const [selectedAgency, setSelectedAgency] = useState<FraudAgency | null>(null);
  ...
  async function submitAsMember() {
    ...
    const created = await insertMemberVerifyLead({
      serviceType: "verify_fraud",
      sourcePage: "/verify/fraud",
      tag: "VERIFY_FRAUD",
      verifyMeta,
      lang,
  ...
  const activeGuidance = selectedAgency ? FRAUD_AGENCY_GUIDANCE[selectedAgency] : null;
  ...
  const pageHeader = getMasterLandingPageHeader(
    ...
    contextTab,
    landingDone
      ? { inQuestions: true, questionDescription: "송금 전 진위 확인부터 피해 발생 후 대응 검토까지" }
      : undefined
  );
  ...
  return (
    ...
    >
        <FunnelPageHeader
          ...
          title={isReviewMaster ? MASTER_LANDING_FRAUD.shortServiceLabel ?? MASTER_LANDING_FRAUD.serviceLabel : pageHeader.title}
          description={
            isReviewMaster
              ? contextTab === "direct"
        ...
        {landingDone && step === "diagnosis" && diagnosis && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <VerifyResultOverviewCards
              diagnosis={diagnosis}
              docCount={getRequiredDocuments("verify_fraud").documents.length}
            />
        ...
        {landingDone && step === "guidanceSelect" && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
              {FRAUD_AGENCY_OPTIONS.map((agency) => (
                <SelectionCard
                  key={agency}
                  title={agency}
