...
import { ArrowLeft, ShieldAlert, CheckCircle2, Paperclip } from "lucide-react";
...
export default function VerifyFraudPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (file && file.size > 0) {
      ...
      const path = `verify-fraud/${leadId}.${safeExt}`;
        ...
        <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
          ...
          {!submitted ? (
            ...
            <div className="mt-5">
              ...
              <Link href="/consultation?case=verify-fraud"
                className="mt-4 inline-flex items-center gap-1.5 rounded-full border border-red-600 px-5 py-2.5 text-sm font-semibold text-red-600 hover:bg-red-50 transition-colors">
                메시지 기다리지 않고 지금 상담하기
              </Link>
            </div>
          )}
