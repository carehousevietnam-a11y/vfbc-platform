...
// STEP5: "고객 공개" 체크박스를 meta.visibleToCustomer(boolean)로 저장한다.
// 이 플래그가 true인 메모만 /api/mypage-data가 골라서 고객에게 보여준다 —
// 체크하지 않으면(기본값) 기존과 동일하게 관리자만 볼 수 있는 내부 메모다.
...
export default async function AdminLeadDetailPage({
  ...
  let permitFileUrl: string | null = null;
  {
    ...
    if (storagePath) {
      try {
        ...
        console.error("[admin/leads] permit Signed URL failed:", err);
      }
    } else if (legacyUrl) {
  ...
  // 첨부 서류 (VERIFY — 신규 storagePath / 과거 file_url fallback)
  ...
  let fileUrl: string | null = null;
  {
    ...
    if (storagePath) {
      try {
        ...
        console.error("[admin/leads] verify attachment Signed URL failed:", err);
      }
    } else if (legacyUrl) {
      fileUrl = legacyUrl;
    }
          ...
          {memoActivities.length === 0 ? (
            ...
            <div className="mt-2 space-y-2">
              {[...memoActivities].reverse().map((m) => {
                const isVisible = asMeta(m.meta)?.visibleToCustomer === true;
                return (
                  <div key={m.id} className="rounded-xl bg-gray-50 px-3 py-2.5">
                    <div className="flex items-center justify-between gap-2">
                      <span
                        className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${
                          isVisible ? "bg-emerald-100 text-emerald-700" : "bg-gray-200 text-gray-600"
                        }`}
                      >
                        {isVisible ? "고객 공개" : "내부 전용"}
                      </span>
                      <p className="text-[11px] text-gray-400">
