      ...
      <div className="w-full xl:pl-[196px]">
        <div className="mx-auto w-full max-w-[1480px] px-5 py-5 sm:px-6 lg:px-8 lg:py-5">
          <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.05)]">
            ...
            <div className="grid border-b border-slate-100 bg-white lg:grid-cols-[1.05fr_1.05fr_.9fr_1.15fr] lg:divide-x lg:divide-slate-100">
              <div className="flex min-h-[126px] flex-col justify-center p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <dl className="mt-2.5 space-y-1.5 text-[11px]">
                      <div className="flex items-baseline justify-between gap-2"><dt className="shrink-0 text-slate-400">연락처</dt><dd className="truncate font-semibold">{lead.phone ?? "-"}</dd></div>
                      <div className="flex items-baseline justify-between gap-2"><dt className="shrink-0 text-slate-400">이메일</dt><dd className="min-w-0 truncate break-all font-semibold">{lead.email ?? "-"}</dd></div>
                    </dl>
                  </div>
                </div>
              </div>
              <div className="flex min-h-[126px] flex-col justify-center gap-3 border-t border-slate-100 p-4 lg:border-t-0">
                <div><dt className="text-[11px] font-semibold tracking-wide text-slate-400">신청 정보</dt><dd className="mt-1 truncate text-[12px] font-semibold">{lead.id}</dd></div>
                <div><dt className="text-[11px] font-semibold tracking-wide text-slate-400">접수일</dt><dd className="mt-1 text-[12px] font-semibold">{new Date(lead.created_at).toLocaleString("ko-KR")}</dd></div>
              </div>
              <div className="flex min-h-[126px] flex-col justify-center gap-3 border-t border-slate-100 p-4 lg:border-t-0">
                <div><dt className="text-[11px] font-semibold tracking-wide text-slate-400">마지막 활동</dt><dd className="mt-1 text-[12px] font-semibold">{latestActivity ? new Date(latestActivity.created_at).toLocaleString("ko-KR") : "-"}</dd></div>
                <div><dt className="text-[11px] font-semibold tracking-wide text-slate-400">접수 상태</dt><dd className={`mt-1 inline-flex rounded-full px-2.5 py-1 text-[11px] font-bold ${consultationStatus.color}`}>{consultationStatus.label}</dd></div>
              </div>
              <div className="flex min-h-[126px] flex-col justify-center border-t border-slate-100 bg-slate-50/50 p-4 lg:border-t-0">
          ...
          <section id="case-progress" className="mt-4 scroll-mt-36 overflow-hidden rounded-2xl border border-blue-200 bg-white shadow-[0_8px_24px_rgba(37,99,235,0.08)]">
            <div className="flex flex-col gap-3 border-b border-blue-100 bg-gradient-to-r from-blue-50 via-white to-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-blue-600 text-white"><ListChecks size={14}/></span>
                  <h2 className="text-[15px] font-extrabold text-slate-950">진행 단계</h2>
                  <span className="rounded-full border border-blue-200 bg-white px-2.5 py-1 text-[10px] font-bold text-blue-700">{processProgress}% 진행</span>
                </div>
                <p className="mt-1.5 text-[11px] text-slate-500">현재 위치와 다음 처리 단계를 먼저 확인하세요.</p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <span className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[11px] font-bold text-slate-700">현재 · {currentStageLabel}</span>
                <span className="rounded-lg bg-blue-600 px-3 py-2 text-[11px] font-bold text-white shadow-sm">다음 · {nextStep?.label ?? "담당자 확인 대기"}</span>
              </div>
            </div>
            <div className="px-5 py-4">
              <div className="flex items-start">
                {processSteps.map((step,i)=>{const isNextStep=i===nextStepIndex;return <div key={step.label} className="relative min-w-0 flex-1 text-center">{i>0&&<div className={`absolute right-1/2 top-[14px] h-[2px] w-full ${step.done?"bg-emerald-300":"bg-slate-200"}`}/>}<div className={`relative z-10 mx-auto flex h-7 w-7 items-center justify-center rounded-full border-2 text-[11px] font-extrabold ${step.done?"border-emerald-300 bg-emerald-50 text-emerald-700":isNextStep?"border-blue-600 bg-blue-600 text-white shadow-[0_0_0_4px_rgba(37,99,235,0.10)]":"border-slate-200 bg-white text-slate-400"}`}>{step.done?<CheckCircle2 size={14}/>:i+1}</div><p className={`mt-2 truncate px-1 text-[10px] font-bold ${step.done?"text-emerald-700":isNextStep?"text-blue-700":"text-slate-500"}`}>{step.label}</p></div>})}
              </div>
              {nextStep&&<div className="mt-4 flex flex-col gap-2 rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-wide text-blue-500">Next Action</p><strong className="mt-0.5 block text-[12px] text-blue-950">{nextStep.label}</strong></div><form action={setProcessStage} className="flex flex-wrap gap-2"><input type="hidden" name="leadId" value={lead.id}/><input type="hidden" name="stageAction" value={nextStep.settableAction??""}/>{nextStep.settableAction==="process_permit_completed"&&<input type="file" name="permitFile" className="max-w-[190px] text-[10px]"/>}<button className="rounded-lg border border-blue-600 bg-white px-3 py-2 text-[11px] font-bold text-blue-700 shadow-sm hover:bg-blue-50">다음 단계로 변경</button></form></div>}
            </div>
          </section>
          ...
          <div className="mt-4 grid min-w-0 gap-3 lg:grid-cols-[minmax(0,2.6fr)_minmax(320px,1fr)] lg:items-stretch">
            <div className="min-w-0 space-y-3.5">
              <section id="case-documents" className="min-w-0 scroll-mt-36 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.05)]">
              ...
              <section id="case-ai-review" className="min-h-[238px] scroll-mt-36 rounded-2xl border border-slate-200 bg-white p-5 pb-6 shadow-[0_1px_3px_rgba(15,23,42,0.05)]">
                <div className="flex flex-wrap items-center gap-2">
