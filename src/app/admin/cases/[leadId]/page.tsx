  ...
  return (
    <main className="min-h-screen bg-[#f4f7fb] text-slate-900">
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-[220px] flex-col bg-[#102b4e] text-white xl:flex">
        ...
        <nav className="flex-1 space-y-1 px-3 py-4 text-[13px]">
          <Link href="/admin/cases" className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-white/70 hover:bg-white/10 hover:text-white"><Home size={16}/>대시보드</Link>
          <div className="rounded-xl bg-white/[0.08] p-1"><div className="flex items-center justify-between rounded-lg bg-white/[0.10] px-3 py-2.5 font-semibold"><span className="flex items-center gap-3"><Inbox size={16}/>신청건 관리</span><ChevronDown size={14}/></div><div className="ml-5 mt-1 border-l border-white/15 py-1 pl-3 text-white/70"><Link href="/admin/cases" className="block rounded-md bg-white/10 px-3 py-2 font-semibold text-white">전체 신청건</Link><span title="전용 필터 페이지 준비 중" className="flex cursor-not-allowed items-center justify-between px-3 py-2 text-white/40">미확인 문서<span className="text-[10px]">준비중</span></span><span title="전용 필터 페이지 준비 중" className="flex cursor-not-allowed items-center justify-between px-3 py-2 text-white/40">보완 요청<span className="text-[10px]">준비중</span></span><span title="전용 필터 페이지 준비 중" className="flex cursor-not-allowed items-center justify-between px-3 py-2 text-white/40">긴급 건<span className="text-[10px]">준비중</span></span></div></div>
          <div title="관리자 문서관리 페이지 준비 중" className="flex cursor-not-allowed items-center justify-between rounded-lg px-3 py-2.5 text-white/40"><span className="flex items-center gap-3"><FolderOpen size={16}/>문서 관리</span><span className="text-[10px]">준비중</span></div><div title="직원관리 페이지 준비 중" className="flex cursor-not-allowed items-center justify-between rounded-lg px-3 py-2.5 text-white/40"><span className="flex items-center gap-3"><Users size={16}/>직원 관리</span><span className="text-[10px]">준비중</span></div><div title="통계 페이지 준비 중" className="flex cursor-not-allowed items-center justify-between rounded-lg px-3 py-2.5 text-white/40"><span className="flex items-center gap-3"><BarChart3 size={16}/>통계 및 리포트</span><span className="text-[10px]">준비중</span></div><Link href="/admin/rejections" className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-white/70 hover:bg-white/10 hover:text-white"><Settings size={16}/>거절이력 관리</Link>
        </nav><div className="border-t border-white/10 p-4 text-[13px] text-white/70"><form action="/api/admin/logout" method="post"><button type="submit" className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left hover:bg-white/10 hover:text-white"><LogOut size={16}/>로그아웃</button></form></div>
      </aside>
      ...
      <div className="w-full xl:pl-[220px]">
        <div className="w-full max-w-none px-5 py-5 sm:px-6 lg:px-8 lg:py-6 2xl:px-10">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><div className="text-[11px] font-medium text-slate-400">신청건 관리　›　신청건 상세</div><div className="mt-2 flex flex-wrap items-center gap-2.5"><h1 className="text-[30px] font-extrabold tracking-tight sm:text-[34px]">신청건 상세</h1><span className={`rounded-full px-3 py-1 text-[11px] font-bold ${categoryInfo.badgeColor}`}>{categoryInfo.label}</span></div></div><Link href="/admin/cases" className="inline-flex w-fit items-center gap-2 rounded-lg border border-slate-200 bg-white px-3.5 py-2 text-[14px] font-semibold shadow-[0_4px_14px_rgba(15,23,42,0.08)]"><ArrowLeft size={14}/>목록으로 돌아가기</Link></div>

          <section className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_4px_14px_rgba(15,23,42,0.08)]">
            <div className="grid lg:grid-cols-[1.25fr_1.05fr_.95fr_1.15fr] lg:divide-x lg:divide-slate-100">
              <div className="p-5 sm:p-6"><div className="flex items-start gap-4"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-700"><User size={24}/></div><div className="min-w-0 flex-1"><p className="text-[13px] font-semibold text-slate-400">고객명</p><p className="mt-0.5 text-[24px] font-extrabold">{lead.name}</p><dl className="mt-4 space-y-2.5 text-[13px]"><div><dt className="text-slate-400">연락처</dt><dd className="font-semibold">{lead.phone ?? "-"}</dd></div><div><dt className="text-slate-400">이메일</dt><dd className="break-all font-semibold">{lead.email ?? "-"}</dd></div><div><dt className="text-slate-400">카카오톡 ID</dt><dd className="font-semibold text-amber-700">{lead.kakao_id ?? "-"}</dd></div><div><dt className="text-slate-400">Zalo ID</dt><dd className="font-semibold text-blue-700">{lead.zalo_id ?? "-"}</dd></div></dl></div></div></div>
              <div className="border-t border-slate-100 p-5 sm:p-6 lg:border-t-0"><p className="text-[13px] font-semibold text-slate-400">서비스</p><div className="mt-1 flex flex-wrap items-center gap-2"><span className="text-[16px] font-bold">{serviceLabel}</span><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${categoryInfo.badgeColor}`}>{categoryInfo.label}</span></div><p className="mt-5 text-[13px] font-semibold text-slate-400">현재 단계</p><span className="mt-1 inline-flex rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700">{currentStageLabel}</span><p className="mt-5 text-[13px] font-semibold text-slate-400">담당 직원</p><div className="mt-1 flex items-center gap-2 text-[14px] font-semibold"><User size={13}/>VFBCAI 담당자</div></div>
              <div className="border-t border-slate-100 p-5 sm:p-6 lg:border-t-0"><dl className="space-y-5 text-[13px]"><div><dt className="text-slate-400">접수일</dt><dd className="mt-1 font-semibold">{new Date(lead.created_at).toLocaleString("ko-KR")}</dd></div><div><dt className="text-slate-400">마지막 활동</dt><dd className="mt-1 font-semibold">{latestActivity ? new Date(latestActivity.created_at).toLocaleString("ko-KR") : "-"}</dd></div></dl></div>
              <div className="border-t border-slate-100 bg-slate-50/50 p-5 lg:border-t-0"><div className="grid grid-cols-2 gap-2.5"><div className="rounded-xl border border-slate-100 bg-white p-3.5"><p className="flex items-center gap-1.5 text-[11px] text-slate-500"><FileText size={13}/>제출 문서</p><p className="mt-1 text-[24px] font-extrabold">{submittedRequiredCount}</p></div><div className="rounded-xl border border-red-100 bg-red-50 p-3.5"><p className="flex items-center gap-1.5 text-[11px] text-red-600"><FileWarning size={13}/>미제출 문서</p><p className="mt-1 text-[24px] font-extrabold text-red-600">{requiredDocuments.length - submittedRequiredCount}</p></div><div className="rounded-xl border border-amber-100 bg-amber-50 p-3.5"><p className="flex items-center gap-1.5 text-[11px] text-amber-700"><Paperclip size={13}/>보완 요청</p><p className="mt-1 text-[24px] font-extrabold text-amber-700">-</p></div><div className="rounded-xl border border-blue-100 bg-blue-50 p-3.5"><p className="flex items-center gap-1.5 text-[11px] text-blue-700"><ShieldCheck size={13}/>AI 점수</p><p className="mt-1 text-[24px] font-extrabold text-blue-800">{typeof activeScore === "number" ? activeScore : "-"}</p></div></div></div>
            </div>
          </section>

          <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_4px_14px_rgba(15,23,42,0.08)] sm:p-6"><div className="flex items-center justify-between"><h2 className="text-[19px] font-extrabold">진행 단계</h2><span className="text-[13px] font-semibold text-slate-400">{currentStageLabel}</span></div><div className="mt-5 flex items-start">{processSteps.map((step,i)=>{const isNextStep=i===nextStepIndex;return <div key={step.label} className="relative min-w-0 flex-1 text-center">{i>0&&<div className={`absolute right-1/2 top-4 h-[2px] w-full ${step.done?"bg-emerald-300":"bg-slate-200"}`}/>}<div className={`relative z-10 mx-auto flex h-8 w-8 items-center justify-center rounded-full border-2 text-[11px] font-extrabold ${step.done?"border-emerald-300 bg-emerald-50 text-emerald-700":isNextStep?"border-blue-600 bg-blue-600 text-white":"border-slate-200 bg-white text-slate-400"}`}>{step.done?<CheckCircle2 size={15}/>:i+1}</div><p className={`mt-2 truncate px-1 text-[10px] font-bold sm:text-[11px] ${step.done?"text-emerald-700":isNextStep?"text-blue-700":"text-slate-500"}`}>{step.label}</p></div>})}</div>{nextStep&&<div className="mt-5 flex flex-col gap-3 rounded-xl bg-blue-50 p-4 sm:flex-row sm:items-center sm:justify-between"><strong className="text-[12px] text-blue-900">다음 단계: {nextStep.label}</strong><form action={setProcessStage} className="flex flex-wrap gap-2"><input type="hidden" name="leadId" value={lead.id}/><input type="hidden" name="stageAction" value={nextStep.settableAction??""}/>{nextStep.settableAction==="process_permit_completed"&&<input type="file" name="permitFile" className="max-w-[190px] text-[10px]"/>}<button className="rounded-lg border border-blue-500 bg-white px-3 py-2 text-[11px] font-bold text-blue-700">다음 단계로 변경</button></form></div>}</section>

          <div className="mt-4 grid min-w-0 gap-4 lg:grid-cols-[minmax(0,68fr)_minmax(360px,32fr)] lg:items-start">
            <div className="min-w-0 space-y-4">
              <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_4px_14px_rgba(15,23,42,0.08)]">
                <div className="flex flex-col gap-3 border-b border-blue-100 bg-blue-50/60 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <h2 className="text-[19px] font-extrabold">고객 제출 문서</h2>
                  ...
                  <span className="inline-flex w-fit items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-[11px] font-semibold text-slate-700 shadow-[0_4px_14px_rgba(15,23,42,0.08)]">
                ...
                <div className="hidden overflow-x-auto sm:block">
                  <table className="w-full min-w-[820px] text-left text-[14px]">
                    <thead className="sticky top-0 z-10 border-b border-slate-100 bg-slate-50 text-slate-500">
                      <tr>
                        <th className="px-5 py-3.5 font-bold">문서명</th>
                        <th className="px-3 py-3.5 font-bold">파일명</th>
                        <th className="px-3 py-3.5 font-bold">구분</th>
                        <th className="px-3 py-3.5 font-bold">제출일</th>
                        <th className="px-3 py-3.5 font-bold">상태</th>
                        <th className="px-3 py-3.5 text-center font-bold">작업</th>
                      </tr>
                    </thead>
                      ...
                      {requiredDocuments.map((label, index) => {
                        ...
                        return (
                          <tr key={label} className="transition-colors odd:bg-white even:bg-slate-50/40 hover:bg-blue-50/50">
                            ...
                            <td className="px-3 py-4 font-medium text-slate-600">{displayFileName}</td>
                            ...
                            <td className="px-3 py-4 text-slate-500">{displayDate}</td>
                            <td className="px-3 py-4">
                            ...
                            <td className="px-3 py-4 text-center">
                              {canOpen ? (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="inline-flex min-w-[52px] items-center justify-center rounded-md border border-slate-200 bg-white px-3 py-1.5 font-bold text-slate-700 shadow-[0_4px_14px_rgba(15,23,42,0.08)] hover:border-blue-300 hover:text-blue-700"
                                >
                                  열기
                                </a>
                              ) : (
                ...
                <div className="divide-y divide-slate-100 sm:hidden">
                  {requiredDocuments.map((label, index) => {
                    ...
                    return (
                      <div key={label} className="flex w-full items-center gap-3 px-5 py-4">
              ...
              <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_4px_14px_rgba(15,23,42,0.08)] sm:p-6">
                <div className="flex flex-wrap items-center gap-2">
                  <h2 className="text-[19px] font-extrabold">AI 진단 결과</h2>
                  <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700">진단 완료</span>
                </div>

                <div className="mt-4 grid overflow-hidden rounded-xl border border-slate-100 sm:grid-cols-[1.65fr_repeat(4,1fr)]">
                  <div className="border-b border-emerald-100 bg-emerald-50/60 p-4 sm:border-b-0 sm:border-r">
                    ...
                    <p className="mt-1 text-[25px] font-extrabold text-emerald-700">{resultInfo?.label ?? "-"}</p>
                ...
                <div className="mt-5 grid gap-5 border-t border-slate-100 pt-4 md:grid-cols-2">
                  ...
                  <div>
                    <div className="flex items-center justify-between gap-3">
                      <h3 className="text-[13px] font-extrabold text-slate-800">권장 조치</h3>
            ...
            <aside className="space-y-4">
              <section className="min-w-0 w-full overflow-hidden rounded-2xl border border-blue-100 bg-white shadow-[0_4px_14px_rgba(15,23,42,0.08)]"><div className="border-b border-blue-100 bg-blue-50 px-5 py-4"><h2 className="text-[17px] font-extrabold text-blue-950">고객 기본 정보</h2></div><dl className="grid grid-cols-[104px_1fr] gap-y-3 p-5 text-[14px]"><dt className="text-slate-500">고객 구분</dt><dd className="text-right font-semibold">개인</dd><dt className="text-slate-500">연락처</dt><dd className="text-right font-semibold">{lead.phone??"-"}</dd><dt className="text-slate-500">이메일</dt><dd className="min-w-0 break-all text-right font-semibold">{lead.email??"-"}</dd><dt className="text-amber-700">카카오톡 ID</dt><dd className="text-right font-bold text-amber-700">{lead.kakao_id??"-"}</dd><dt className="text-blue-700">Zalo ID</dt><dd className="text-right font-bold text-blue-700">{lead.zalo_id??"-"}</dd><dt className="text-slate-500">접수일</dt><dd className="text-right font-semibold">{new Date(lead.created_at).toLocaleDateString("ko-KR")}</dd></dl></section>
              <section className="min-w-0 w-full overflow-hidden rounded-2xl border border-violet-100 bg-white shadow-[0_4px_14px_rgba(15,23,42,0.08)]"><div className="flex items-center justify-between border-b border-violet-100 bg-violet-50 px-5 py-4"><h2 className="text-[17px] font-extrabold text-violet-950">담당자 정보</h2><span className="rounded-md border border-blue-200 px-2 py-1 text-[10px] font-bold text-blue-700">담당자 변경</span></div><dl className="grid grid-cols-[104px_1fr] gap-y-3 p-5 text-[14px]"><dt className="text-slate-500">담당 직원</dt><dd className="text-right font-bold text-blue-700">VFBCAI 담당자</dd><dt className="text-slate-400">소속 팀</dt><dd className="text-right font-semibold">행정전문팀</dd><dt className="text-slate-400">배정일</dt><dd className="text-right font-semibold">-</dd></dl></section>
              <section className="min-w-0 w-full overflow-hidden rounded-2xl border border-amber-100 bg-white shadow-[0_4px_14px_rgba(15,23,42,0.08)]"><div className="flex items-center justify-between border-b border-amber-100 bg-amber-50 px-5 py-4"><h2 className="text-[17px] font-extrabold text-amber-950">내부 메모</h2><span className="rounded-md border border-blue-200 px-2 py-1 text-[10px] font-bold text-blue-700">메모 작성</span></div><div className="p-5">{memoActivities.length>0&&<div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-[13px]">{String(asMeta(memoActivities[memoActivities.length-1].meta)?.memo??"")}</div>}<form action={addExpertMemo} className="mt-3"><input type="hidden" name="leadId" value={lead.id}/><textarea name="memo" required rows={3} placeholder="고객 및 업무 관련 메모를 작성하세요" className="w-full resize-none rounded-lg border border-slate-200 p-3 text-[11px] outline-none focus:border-blue-500"/><button className="mt-2 rounded-lg bg-blue-600 px-3 py-2 text-[11px] font-bold text-white">메모 저장</button></form></div></section>
              <section className="min-w-0 w-full overflow-hidden rounded-2xl border border-red-100 bg-white shadow-[0_4px_14px_rgba(15,23,42,0.08)]"><div className="border-b border-red-100 bg-red-50 px-5 py-4"><h2 className="text-[17px] font-extrabold text-red-950">타 기관 거절 이력</h2></div><div className="p-5">{rejections.length?<div className="mt-3 space-y-2">{rejections.map(r=><div key={r.id} className="rounded-lg border border-red-100 p-3 text-[11px]"><div className="flex justify-between"><strong>{getServiceLabel(r.service_type)}</strong><span className="text-red-500">거절</span></div><p className="mt-1 text-slate-500">{r.reason||"사유 미기재"}</p></div>)}</div>:<p className="mt-3 text-[12px] text-slate-400">연결된 거절이력이 없습니다.</p>}</div></section>
              <section className="min-w-0 w-full overflow-hidden rounded-2xl border border-emerald-100 bg-white shadow-[0_4px_14px_rgba(15,23,42,0.08)]"><div className="border-b border-emerald-100 bg-emerald-50 px-5 py-4"><h2 className="text-[17px] font-extrabold text-emerald-950">활동 타임라인</h2></div><div className="p-5">{activities.length?<div className="relative mt-4 space-y-3 before:absolute before:bottom-1 before:left-[5px] before:top-1 before:w-px before:bg-slate-200">{activities.slice(-6).map(a=><div key={a.id} className="relative grid grid-cols-[92px_1fr] gap-2 pl-4 text-[10px]"><span className={`absolute left-0 top-1 h-[11px] w-[11px] rounded-full ring-2 ring-white ${getActivityDotColor(a.action)}`}/><span className="text-slate-400">{new Date(a.created_at).toLocaleString("ko-KR")}</span><div><strong className="text-blue-700">{getActivityLabel(a.action)}</strong>{a.tag&&<p className="mt-0.5 truncate text-slate-500">{a.tag}</p>}</div></div>)}</div>:<p className="mt-3 text-[12px] text-slate-400">기록된 활동이 없습니다.</p>}</div></section>
            </aside>
          </div>

          <section className="mt-4 w-full rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_4px_14px_rgba(15,23,42,0.08)] sm:p-6"><div className="flex items-center gap-2"><MessageSquareText size={16} className="text-blue-700"/><h2 className="text-[14px] font-extrabold">전문가 상담 요청 (Case Room)</h2></div>{consultationRequests.length===0?<p className="mt-3 text-[12px] text-slate-400">접수된 상담 요청이 없습니다.</p>:<div className="mt-4 space-y-3">{[...consultationRequests].reverse().map(req=>{const response=findResponseFor(req.id);return <div key={req.id} className="rounded-xl bg-slate-50 p-4"><div className="flex justify-between text-[11px]"><strong>고객 문의</strong><span className={response?"text-emerald-600":"text-amber-600"}>{response?"답변 완료":"미답변"}</span></div><p className="mt-2 whitespace-pre-wrap text-[13px]">{String(asMeta(req.meta)?.content??"")}</p>{response?<div className="mt-3 rounded-lg bg-blue-50 p-3 text-[12px] text-blue-900">{String(asMeta(response.meta)?.content??"")}</div>:<form action={respondToConsultation} className="mt-3"><input type="hidden" name="leadId" value={lead.id}/><input type="hidden" name="requestActivityId" value={req.id}/><textarea name="content" required rows={3} className="w-full resize-none rounded-lg border border-slate-200 bg-white p-3 text-[12px]" placeholder="답변을 입력하세요"/><button className="mt-2 rounded-lg bg-blue-600 px-3 py-2 text-[11px] font-bold text-white">답변 등록</button></form>}</div>})}</div>}</section>
        </div>
      </div>
    </main>
  );
