  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[#f4f7fb] text-slate-900">
      ...
      <div className="w-full xl:pl-[196px]">
        <div className="w-full max-w-none px-4 py-5 sm:px-5 lg:px-6 2xl:px-8 lg:py-5">
          ...
          <div className="mt-4 grid min-w-0 gap-3 lg:grid-cols-[minmax(0,2.6fr)_minmax(320px,1fr)] lg:items-stretch">
            ...
            <aside className="flex h-full min-w-0 flex-col gap-3.5">
              ...
              <section className="flex min-h-[240px] min-w-0 w-full flex-1 flex-col overflow-hidden rounded-2xl border border-emerald-100 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.05)]"><div className="shrink-0 border-b border-emerald-100 bg-emerald-50 px-4 py-3"><h2 className="text-[14px] font-bold text-emerald-950">활동 타임라인</h2></div><div className="flex-1 px-4 py-3.5">{activities.length?<div className="relative space-y-3.5 before:absolute before:bottom-1 before:left-[4px] before:top-1 before:w-px before:bg-slate-200">{activities.slice(-6).map(a=><div key={a.id} className="relative grid grid-cols-[86px_1fr] gap-2.5 pl-4 text-[10px]"><span className={`absolute left-0 top-1 h-[9px] w-[9px] rounded-full ring-2 ring-white ${getActivityDotColor(a.action)}`}/><span className="text-slate-400">{new Date(a.created_at).toLocaleString("ko-KR")}</span><div><strong className="text-[11px] text-blue-700">{getActivityLabel(a.action)}</strong>{a.tag&&<p className="mt-0.5 truncate text-slate-500">{a.tag}</p>}</div></div>)}</div>:<p className="text-[11px] text-slate-400">기록된 활동이 없습니다.</p>}</div></section>
            </aside>
          </div>

          <section className="mt-3 min-h-[64px] w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-[0_1px_3px_rgba(15,23,42,0.05)]"><div className="flex items-center gap-2"><MessageSquareText size={16} className="text-blue-700"/><h2 className="text-[14px] font-bold">전문가 상담 요청 (Case Room)</h2></div>{consultationRequests.length===0?<div className="mt-1 flex min-h-[128px] flex-col items-center justify-center gap-2.5 rounded-xl border border-dashed border-slate-200 bg-slate-50/50 px-4 py-8 text-center"><div className="flex h-10 w-10 items-center justify-center rounded-full bg-white text-slate-300 shadow-[0_1px_3px_rgba(15,23,42,0.05)]"><MessageSquareText size={18}/></div><p className="text-[13px] font-semibold text-slate-500">접수된 상담 요청이 없습니다</p><p className="text-[11px] text-slate-400">고객이 Case Room에서 상담을 요청하면 이곳에 표시됩니다.</p></div>:<div className="mt-4 space-y-3">{[...consultationRequests].reverse().map(req=>{const response=findResponseFor(req.id);return <div key={req.id} className="rounded-xl bg-slate-50 p-4"><div className="flex justify-between text-[11px]"><strong>고객 문의</strong><span className={response?"text-emerald-600":"text-amber-600"}>{response?"답변 완료":"미답변"}</span></div><p className="mt-2 whitespace-pre-wrap text-[11px]">{String(asMeta(req.meta)?.content??"")}</p>{response?<div className="mt-3 rounded-lg bg-blue-50 p-3 text-[12px] text-blue-900">{String(asMeta(response.meta)?.content??"")}</div>:<form action={respondToConsultation} className="mt-3"><input type="hidden" name="leadId" value={lead.id}/><input type="hidden" name="requestActivityId" value={req.id}/><textarea name="content" required rows={3} className="w-full resize-none rounded-lg border border-slate-200 bg-white p-3 text-[12px]" placeholder="답변을 입력하세요"/><button className="mt-2 h-8 rounded-lg bg-blue-600 px-3.5 text-[11px] font-bold text-white">답변 등록</button></form>}</div>})}</div>}</section>
        </div>
      </div>
    </main>
  );
