    ...
    <main className="min-h-screen overflow-x-hidden bg-[#f4f7fb] text-slate-900">
      ...
      <div className="w-full xl:pl-[196px]">
        <div className="mx-auto w-full max-w-[1280px] px-5 py-5 sm:px-6 lg:px-8 lg:py-5">
          ...
          <div className="mt-4 grid min-w-0 gap-3 lg:grid-cols-[minmax(0,2.6fr)_minmax(320px,1fr)] lg:items-start">
            ...
            <aside className="space-y-3.5">
              ...
              <section className="min-h-[192px] min-w-0 w-full overflow-hidden rounded-2xl border border-emerald-100 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.05)]"><div className="border-b border-emerald-100 bg-emerald-50 px-4 py-3"><h2 className="text-[14px] font-bold text-emerald-950">활동 타임라인</h2></div><div className="px-4 py-3.5">{activities.length?<div className="relative space-y-3.5 before:absolute before:bottom-1 before:left-[4px] before:top-1 before:w-px before:bg-slate-200">{activities.slice(-6).map(a=><div key={a.id} className="relative grid grid-cols-[86px_1fr] gap-2.5 pl-4 text-[10px]"><span className={`absolute left-0 top-1 h-[9px] w-[9px] rounded-full ring-2 ring-white ${getActivityDotColor(a.action)}`}/><span className="text-slate-400">{new Date(a.created_at).toLocaleString("ko-KR")}</span><div><strong className="text-[11px] text-blue-700">{getActivityLabel(a.action)}</strong>{a.tag&&<p className="mt-0.5 truncate text-slate-500">{a.tag}</p>}</div></div>)}</div>:<p className="text-[11px] text-slate-400">기록된 활동이 없습니다.</p>}</div></section>
            </aside>
          </div>
