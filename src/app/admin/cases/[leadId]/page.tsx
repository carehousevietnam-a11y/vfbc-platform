  ...
  return (
    <main className="min-h-screen bg-[#f4f7fb] text-slate-900">
      ...
      <div className="w-full xl:pl-[196px]">
        <div className="mx-auto w-full max-w-[1480px] px-5 py-5 sm:px-6 lg:px-7 lg:py-6 2xl:px-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><div className="text-[11px] font-medium text-slate-400">신청건 관리　›　신청건 상세</div><div className="mt-2 flex flex-wrap items-center gap-2.5"><h1 className="text-[26px] font-extrabold tracking-tight sm:text-[30px]">신청건 상세</h1><span className={`rounded-full px-3 py-1 text-[11px] font-bold ${categoryInfo.badgeColor}`}>{categoryInfo.label}</span></div></div><Link href="/admin/cases" className="inline-flex w-fit items-center gap-2 rounded-lg border border-slate-200 bg-white px-3.5 py-2 text-[13px] font-semibold shadow-[0_1px_3px_rgba(15,23,42,0.08)]"><ArrowLeft size={14}/>목록으로 돌아가기</Link></div>

          <section className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.08)]">
            <div className="grid lg:grid-cols-[1.2fr_1.05fr_.9fr_1.1fr] lg:divide-x lg:divide-slate-100">
              <div className="p-5 sm:p-6"><div className="flex items-start gap-4"><div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-700"><User size={24}/></div><div className="min-w-0 flex-1"><p className="text-[12px] font-semibold text-slate-400">고객명</p><p className="mt-0.5 text-[21px] font-extrabold">{lead.name}</p><dl className="mt-4 space-y-2.5 text-[13px]"><div><dt className="text-slate-400">연락처</dt><dd className="font-semibold">{lead.phone ?? "-"}</dd></div><div><dt className="text-slate-400">이메일</dt><dd className="break-all font-semibold">{lead.email ?? "-"}</dd></div><div><dt className="text-slate-400">카카오톡 ID</dt><dd className="font-semibold text-amber-700">{lead.kakao_id ?? "-"}</dd></div><div><dt className="text-slate-400">Zalo ID</dt><dd className="font-semibold text-blue-700">{lead.zalo_id ?? "-"}</dd></div></dl></div></div></div>
              <div className="border-t border-slate-100 p-5 sm:p-6 lg:border-t-0"><p className="text-[12px] font-semibold text-slate-400">서비스</p><div className="mt-1 flex flex-wrap items-center gap-2"><span className="text-[16px] font-bold">{serviceLabel}</span><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${categoryInfo.badgeColor}`}>{categoryInfo.label}</span></div><p className="mt-5 text-[12px] font-semibold text-slate-400">현재 단계</p><span className="mt-1 inline-flex rounded-full bg-emerald-50 px-2.5 py-1 text-[11px] font-bold text-emerald-700">{currentStageLabel}</span><p className="mt-5 text-[12px] font-semibold text-slate-400">담당 직원</p><div className="mt-1 flex items-center gap-2 text-[13px] font-semibold"><User size={13}/>VFBCAI 담당자</div></div>
          ...
          <div className="mt-4 grid min-w-0 gap-3.5 lg:grid-cols-[minmax(0,1.9fr)_minmax(330px,0.9fr)] lg:items-start">
            <div className="min-w-0 space-y-3.5">
              <section className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.08)]">
                <div className="flex flex-col gap-3 border-b border-blue-100 bg-blue-50/60 px-4 py-2.5.5 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                ...
                <div className="divide-y divide-slate-100 sm:hidden">
                  {requiredDocuments.map((label, index) => {
                    ...
                    return (
                      <div key={label} className="flex w-full items-center gap-3 px-4 py-2.5.5">
