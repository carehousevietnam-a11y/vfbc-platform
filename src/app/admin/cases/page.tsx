          ...
          {dayLeads.map((lead) => {
            ...
            return (
              <LeadMobileCard
                ...
                returnTo={`/admin/cases?category=${category}&service=${service}&date=${date}`}
              />
            );
          })}
        </div>
  ...
  return (
    <Shell status={status}>
      <PageHeader
        ...
        description={
          status
            ? `${currentViewLabel} 업무만 모아 확인합니다. 현재 적용된 조건과 검색 결과를 아래에서 확인하세요.`
...
function buildCaseDetailHref(leadId: string, returnTo: string) {
  const params = new URLSearchParams();
  params.set("returnTo", returnTo.startsWith("/admin/cases") ? returnTo : "/admin/cases");
  return `/admin/cases/${leadId}?${params.toString()}`;
}
