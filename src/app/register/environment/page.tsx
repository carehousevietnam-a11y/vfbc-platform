...
// 환경허가(환경영향평가·배출허가)는 관할 지역·업종·규모에 따라 담당부서가
...
// 자체 진단 로직 (checkDiagnosis.ts 미사용, 규칙 기반) — 등록상태·배출·방지시설
// 확보 여부로 점수를 계산하고, 환경영향평가서 준비상태는 체크리스트 항목으로만
// 반영한다. 법 조항·구체적 허가가능 여부는 단정하지 않고 "가능성" 톤을 유지한다.
...
function computeEnvironmentDiagnosis(
  ...
  const checklist = [
    { label: "사업자·법인(사업장) 등록 서류 준비", passed: registrationStatus === "confirmed" },
  ...
  return {
    ...
    checklist,
    note,
    estimatedDays: { min: 25, max: 50 },
  };
}

function DiagnosisReportCard({ diagnosis }: { diagnosis: EnvironmentDiagnosis }) {
...
export default function RegisterEnvironmentPage() {
  ...
  const showResult = Boolean(
    environmentChoice && !isUnlicensedOperating && registrationStatus && facilityStatus && assessmentStatus
  );
    ...
    <main className="min-h-screen bg-[#fafafa]">
      ...
      <div className="mx-auto max-w-xl px-6 py-10">
        ...
        {!rejectionStepDone && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              1. 이전에 다른 곳(정부기관 또는 타 대행사)에서 환경허가(환경영향평가·
              배출허가)를 신청하셨다가 거절·반려되신 적이 있나요?
            </p>
        ...
        {rejectionStepDone && !environmentChoice && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              2. 현재 사업장을 어떻게 가동하고 계신가요?
            </p>
            <div className="mt-4 grid gap-3 sm:grid-cols-1">
              {[
                { key: "not_started", label: "아직 가동 전입니다", desc: "허가 신청을 준비하는 단계" },
                { key: "operating_licensed", label: "허가를 받고 정상 가동 중입니다", desc: "" },
                { key: "operating_unlicensed", label: "허가 없이 이미 가동 중입니다", desc: "" },
              ].map((opt) => (
                <button
                  key={opt.key}
                  onClick={() => setEnvironmentChoice(opt.key as EnvironmentChoice)}
        ...
        {rejectionStepDone && isUnlicensedOperating && (
          <div className="mt-8 rounded-3xl bg-white border border-red-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-4 text-lg font-bold text-gray-900">
              무허가 가동은 즉시 중단될 수 있습니다
            </p>
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              환경허가 없이 가동 중인 경우 단속 시 즉시 가동중단 조치될
              수 있으며, 이후 정식 허가 신청에도 불이익이 있을 수
              있습니다. 가능한 빨리 허가 절차를 진행하시길 권합니다.
            </p>
            <div className="mt-5 flex flex-col gap-3">
              <button
                onClick={() => setEnvironmentChoice(null)}
          ...
          <div className="mt-8">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {[
                ...
                <button
                  key={opt.key}
                  onClick={() => setAssessmentStatus(opt.key as AssessmentStatus)}
                  className="rounded-2xl bg-white border border-gray-100 p-4 text-sm font-semibold text-gray-900 shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:-translate-y-0.5 transition-all"
                >
                  {opt.label}
                </button>
        ...
        {showResult && result === "possible" && !leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              현재 사업자등록·배출시설 준비 상태 기준으로 환경허가(환경영향평가
              ·배출허가) 신청 요건을 충족합니다.
            </p>
        ...
        {showResult && result === "possible" && leadSubmitted && !agencyRequested && !detailStage && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <div className="mt-4 rounded-xl bg-gray-50 px-4 py-3">
              <p className="text-xs font-semibold text-gray-700">환경허가에 필요한 서류</p>
              ...
              <p className="mt-2 text-[11px] text-gray-400">
                정확한 요건은 업종·배출규모에 따라 다를 수 있어 담당자
                확인이 필요합니다.
              </p>
            </div>
            ...
            <div className="mt-3 flex flex-col gap-3">
              <a
                href={REGISTER_ENVIRONMENT_OFFICIAL_URL}
                target="_blank"
            ...
            <p className="mt-2 text-[11px] text-gray-400">
              ...
              접속 후 관할 지역과 환경 담당부서를 선택하시면 신청 메뉴를
              찾으실 수 있습니다.
            </p>
        ...
        {showResult && result === "possible" && leadSubmitted && !agencyRequested && detailStage && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            <CheckCircle2 className="text-emerald-600" size={28} />
            <p className="mt-4 text-lg font-bold text-gray-900">환경허가 진행 서류 및 절차</p>
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="rounded-xl bg-gray-50 px-4 py-3">
                <p className="text-xs font-semibold text-gray-700">② 시설·평가자료 준비</p>
                <ul className="mt-2 space-y-2">
                  <li className="text-xs text-gray-600 pl-1">
                    · 배출시설·방지시설 설치 확인
                  </li>
                  <li className="text-xs text-gray-600 pl-1">
                    · 환경영향평가서(또는 환경보호계획서) 작성
                  </li>
                  {assessmentStatus === "not_ready" && (
                    <li className="text-xs text-gray-600 pl-1">
                      · 평가자료가 아직 준비되지 않아, 자료 보완 후
                      재검토가 필요할 수 있습니다 — 이 부분은 대행 접수 시
                      담당자가 우선 확인해드립니다.
                    </li>
                  )}
                </ul>
              ...
              <div className="rounded-xl bg-gray-50 px-4 py-3">
                ...
                <p className="mt-2 text-xs text-gray-600 leading-relaxed">
                  서류 준비 → 배출·방지시설 설치 확정 → 환경영향평가(또는
                  환경보호계획서) 신청·접수 → 심사·현장확인 → 허가증 발급
                  → 가동 개시
                </p>
                <p className="mt-2 text-[11px] text-gray-400">
                  * 위 절차는 일반적인 흐름 안내이며, 업종·배출규모에 따라
                  순서나 요건이 달라질 수 있습니다.
                </p>
              </div>
            </div>
        ...
        {showResult && result === "conditional" && !leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-amber-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              현재 사업자등록 또는 배출시설 준비 상태만으로는 환경허가
        ...
        {showResult && result === "conditional" && leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-amber-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <Link
              href="/consultation?case=register-environment-conditional"
