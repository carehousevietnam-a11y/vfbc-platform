...
type AssessmentStatus = "ready" | "not_ready" | null;
type EnvironmentChoice = "not_started" | "operating_licensed" | "operating_unlicensed" | null;
...
const OPERATION_OPTIONS: { key: NonNullable<EnvironmentChoice>; label: string; desc: string; icon: typeof Store; tone: SelectionCardTone }[] = [
  { key: "not_started", label: "아직 가동 전입니다", desc: "환경허가 신청을 준비하는 단계입니다.", icon: Store, tone: "blue" },
  { key: "operating_licensed", label: "허가를 받고 정상 가동 중입니다", desc: "현재 허가 상태를 다시 확인합니다.", icon: CheckCircle2, tone: "green" },
  { key: "operating_unlicensed", label: "허가 없이 이미 가동 중입니다", desc: "무허가 가동 위험을 우선 확인합니다.", icon: AlertTriangle, tone: "red" },
];
...
const PREMISES_OPTIONS: { key: NonNullable<FacilityStatus>; label: string; desc: string; icon: typeof Store; tone: SelectionCardTone }[] = [
  { key: "secured", label: "설치 완료", desc: "배출시설·방지시설 설치를 완료했습니다.", icon: Store, tone: "green" },
  { key: "unsecured", label: "아직 미설치", desc: "배출시설·방지시설 설치가 남아 있습니다.", icon: FileWarning, tone: "amber" },
];

const HYGIENE_OPTIONS: { key: NonNullable<AssessmentStatus>; label: string; desc: string; icon: typeof Flame; tone: SelectionCardTone }[] = [
  { key: "ready", label: "예, 준비했습니다", desc: "환경영향평가서 또는 환경보호계획서를 준비했습니다.", icon: CheckCircle2, tone: "green" },
  { key: "not_ready", label: "아직입니다", desc: "환경 관련 평가서 또는 계획서 준비가 남아 있습니다.", icon: Flame, tone: "amber" },
];
...
function computeEnvironmentDiagnosis(
  ...
  const checklist = [
    ...
    { label: "배출시설·방지시설 설치 확보", passed: facilityStatus === "secured" },
    { label: "환경영향평가서(또는 환경보호계획서) 준비", passed: assessmentStatus === "ready" },
  ];
...
// 확인하여 구조·className·순서를 옮겨왔다. 값만 REGISTER(environment) 진단 결과로 채운다.

// TRC의 PremiumLeadCapture 안 원형 게이지와 100% 동일한 마크업 — 결과 미리보기(가입 전)용.
function EnvironmentScoreGauge({
  score,
  tone,
}: {
  score: number;
...
// 추가하지 않는다.
function EnvironmentResultOverviewCards({
  diagnosis,
  docCount,
}: {
  diagnosis: EnvironmentDiagnosis;
  ...
  const items = [
    {
      n: 1,
      label: "허가 가능성",
      visual: <EnvironmentScoreGauge score={feasibilityScore} tone={resultTone} />,
...
function EnvironmentDesktopResultHeader({
  diagnosis,
}: {
  diagnosis: EnvironmentDiagnosis;
}) {
...
function EnvironmentResultSummaryCard({ diagnosis }: { diagnosis: EnvironmentDiagnosis }) {
...
// 길이·모바일/PC 배열을 그대로 일치시킨 3버튼 CTA. 서비스명(환경허가)과 연결
// 대상(공식 사이트 URL, environment CRM)만 Environment에 맞게 대체했다.
...
function EnvironmentNextStepOptions({
  onSelf,
  onExpert,
  onAiReport,
  officialUrl,
...
function EnvironmentLeadCapture({
  diagnosis,
  messengers,
  lang,
  fieldErrors,
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-4 text-lg font-bold text-gray-900">
              {isPossible ? "환경허가 진행이 가능합니다" : "보완이 필요할 수 있습니다"}
            </p>

            <p className="mt-2 text-sm leading-relaxed text-gray-600">
              {isPossible
                ? "현재 사업자등록·배출시설 준비 상태 기준으로 환경허가 신청 요건을 충족합니다."
                : "현재 사업자등록 또는 배출시설 준비 상태만으로는 환경허가 신청이 자동으로 진행되지 않습니다. 준비 서류를 보완하면 진행할 수 있는 경우가 많습니다."}
            </p>
          </div>

          <EnvironmentScoreGauge score={diagnosis.feasibilityScore} tone={diagnosis.resultTone} />
        </div>
...
export default function RegisterEnvironmentPage() {
  const [environmentChoice, setEnvironmentChoice] = useState<EnvironmentChoice>(null);
  ...
  const [assessmentStatus, setAssessmentStatus] = useState<AssessmentStatus>(null);
  ...
  function applyRestoredRegister(restored: RestoredRegisterLead) {
    ...
    if (meta) {
      if (meta.environmentChoice) setEnvironmentChoice(meta.environmentChoice as EnvironmentChoice);
      ...
      if (meta.assessmentStatus) setAssessmentStatus(meta.assessmentStatus as AssessmentStatus);
  ...
  async function handleLandingContinue() {
    const { loggedIn, restored } = await loadRegisterMemberEntryState(
      ...
      "register_environment_diagnosis_lead",
      { allowRestore: true }
    );
  ...
  const isUnlicensedOperating = environmentChoice === "operating_unlicensed";
  ...
  const result: ResultTone =
    registrationStatus && facilityStatus
      ? registrationStatus === "confirmed" && facilityStatus === "secured"
        ...
        : "conditional"
      : null;
  const showResult = Boolean(environmentChoice && !isUnlicensedOperating && registrationStatus && facilityStatus && assessmentStatus);
  ...
  const diagnosis = canShowResults
    ? computeEnvironmentDiagnosis(registrationStatus, facilityStatus, assessmentStatus)
    : null;

  const requiredDocs = getRequiredDocuments("register_environment");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const created = await submitMemberRegisterLead({
        serviceType: "register_environment",
        sourcePage: "/register/environment",
        result,
        diagnosisAction: "register_environment_diagnosis_lead",
        tag: "REGISTER_ENVIRONMENT",
        meta: {
    ...
    diagnosis,
    result,
    environmentChoice, registrationStatus, facilityStatus, assessmentStatus,
    previousRejection,
    rejectionReason,
    lang,
  ...
  function recordRejectionAnonymously() {
    ...
    pendingRejectionInsertRef.current = supabase
      ...
      .insert({
        id,
        service_type: "register_environment",
        source_page: "/register/environment",
        reason: null,
      })
      .then(({ error }) => {
        if (error) {
  ...
  async function handleExpertRequest() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=register_environment&mode=expert`;
        return;
      }
  ...
  async function handleAiReportRequest() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=register_environment&mode=ai_report`;
        return;
      }
  ...
  async function handleLeadSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    await supabase.from("crm_activities").insert({
      lead_id: newLeadId,
      action: "register_environment_diagnosis_lead",
      tag: "REGISTER_ENVIRONMENT",
      meta: diagnosis
        ? {
            feasibilityScore: diagnosis.feasibilityScore,
    ...
    >
      <FunnelPageHeader
        ...
        description={
          !costEntryDone
            ? contextTab === "review"
              ? "현재 받은 안내나 견적이 환경허가에 필요한 절차와 비용 기준에 맞는지 확인합니다."
              : contextTab === "direct"
                ? "환경허가의 기본 절차·준비 항목·추가 절차 가능성을 확인합니다."
      ...
      />
        ...
        {!restoreRegisterPending && !costEntryDone && (
          <MasterFunnelLanding
            config={MASTER_LANDING_ENVIRONMENT}
            ...
            <VerifyStepLayout
              ...
              question={
                <>
                  <QuestionSection
                    step={1}
                    title="이전에 다른 곳(정부기관 또는 타 대행사)에서 환경허가를 신청하셨다가 거절·반려되신 적이 있나요?"
            ...
            {previousRejection === true && (
              <div className="mt-4">
                ...
                <textarea
                  ...
                  placeholder={
                    "예)\n- 사업자등록 서류가 미비하다고 들었습니다.\n- 환경영향평가 또는 배출시설 자료가 부족하다고 들었습니다.\n- 정확한 이유를 듣지 못했습니다.\n\n자유롭게 작성해주세요(선택)."
                  }
                  rows={6}
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && !environmentChoice && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="register"
              step={2}
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && isUnlicensedOperating && (
          <div className={`mt-8 ${FUNNEL_QUESTION_COLUMN}`}>
            ...
            <div className="mt-5 flex flex-col gap-3">
              ...
              <Link
                href="/consultation?case=register-environment-unlicensed-warning"
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && environmentChoice && !isUnlicensedOperating && !registrationStatus && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="register"
              step={3}
              question={
                <QuestionSection
                  step={3}
                  title="사업자·법인(사업장) 등록 서류가 준비되어 있나요?"
                  description="환경허가 신청에 앞서 확인하는 기본 등록 서류입니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
          ...
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              ...
              question={
                <QuestionSection
                  step={4}
                  title="배출시설·방지시설을 설치하셨나요?"
                  description="허가 신청 시 확인할 시설 준비 상태입니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && registrationStatus && facilityStatus && !assessmentStatus && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="register"
              step={4}
              question={
                <QuestionSection
                  step={5}
                  title="환경영향평가서(또는 환경보호계획서)를 준비하셨나요?"
                  description="환경허가 관련 평가·계획서 준비 상태를 확인합니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
        ...
        {!restoreRegisterPending && costEntryDone && canShowResults && diagnosis && leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-5 sm:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <EnvironmentDesktopResultHeader diagnosis={diagnosis} />

            <EnvironmentResultOverviewCards diagnosis={diagnosis} docCount={requiredDocs.documents.length} />

            <EnvironmentResultSummaryCard diagnosis={diagnosis} />
            ...
            <p className="mt-2 text-[11px] text-gray-400">
              공공서비스포털(Cổng Dịch vụ công quốc gia)의 환경허가 절차 안내
