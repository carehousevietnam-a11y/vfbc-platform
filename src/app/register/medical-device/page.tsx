...
const REGISTER_QUESTION_CONTEXT = "의료기기 수입·유통허가 가능성 진단";
...
const OPERATION_OPTIONS: { key: NonNullable<MedicalDeviceChoice>; label: string; desc: string; icon: typeof Store; tone: SelectionCardTone }[] = [
  { key: "not_started", label: "아직 수입·유통 전입니다", desc: "의료기기 허가를 준비하는 단계입니다.", icon: Store, tone: "blue" },
  { key: "distributing_licensed", label: "허가를 받고 유통 중입니다", desc: "현재 허가 상태를 다시 확인합니다.", icon: CheckCircle2, tone: "green" },
  { key: "distributing_unlicensed", label: "허가 없이 이미 유통 중입니다", desc: "무허가 유통 위험을 우선 확인합니다.", icon: AlertTriangle, tone: "red" },
];
...
const PREMISES_OPTIONS: { key: NonNullable<FacilityStatus>; label: string; desc: string; icon: typeof Store; tone: SelectionCardTone }[] = [
  { key: "secured", label: "확보 완료", desc: "의료기기 보관·유통시설을 확보했습니다.", icon: Store, tone: "green" },
  { key: "unsecured", label: "아직 미확보", desc: "보관·유통시설 확보가 남아 있습니다.", icon: FileWarning, tone: "amber" },
];

const HYGIENE_OPTIONS: { key: NonNullable<QualityDocStatus>; label: string; desc: string; icon: typeof Flame; tone: SelectionCardTone }[] = [
  { key: "ready", label: "예, 준비했습니다", desc: "제품 분류와 품질서류를 준비했습니다.", icon: CheckCircle2, tone: "green" },
  { key: "not_ready", label: "아직입니다", desc: "제품 분류 또는 품질서류 준비가 남아 있습니다.", icon: Flame, tone: "amber" },
];
...
// 확인하여 구조·className·순서를 옮겨왔다. 값만 REGISTER(medical-device) 진단 결과로 채운다.

// TRC의 PremiumLeadCapture 안 원형 게이지와 100% 동일한 마크업 — 결과 미리보기(가입 전)용.
function MedicalDeviceScoreGauge({
  score,
  tone,
}: {
  score: number;
...
function ResultHeaderGauge({
  ...
  diagnosis: MedicalDeviceDiagnosis;
  size?: number;
}) {
...
// 추가하지 않는다.
function MedicalDeviceResultOverviewCards({
  diagnosis,
  docCount,
}: {
  diagnosis: MedicalDeviceDiagnosis;
  ...
  const items = [
    {
      n: 1,
      label: "허가 가능성",
      visual: <MedicalDeviceScoreGauge score={feasibilityScore} tone={resultTone} />,
...
function MedicalDeviceDesktopResultHeader({
  diagnosis,
}: {
  diagnosis: MedicalDeviceDiagnosis;
}) {
      ...
      <div className="flex min-w-0 items-start gap-4">
        ...
        <div className="min-w-0">
          ...
          <h2 className="mt-1 text-xl font-bold tracking-tight text-gray-900">
            {isPossible
              ? "의료기기 수입·유통허가 진행 가능성이 높습니다"
              : "의료기기 수입·유통허가 진행 전 추가 확인이 필요합니다"}
          </h2>
...
function MedicalDeviceResultSummaryCard({ diagnosis }: { diagnosis: MedicalDeviceDiagnosis }) {
...
// 길이·모바일/PC 배열을 그대로 일치시킨 3버튼 CTA. 서비스명(의료기기 수입·유통허가)과 연결
// 대상(공식 사이트 URL, medical-device CRM)만 MedicalDevice에 맞게 대체했다.
...
function MedicalDeviceNextStepOptions({
  onSelf,
  onExpert,
  onAiReport,
  officialUrl,
...
function MedicalDeviceLeadCapture({
  diagnosis,
  messengers,
  lang,
  fieldErrors,
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-4 text-lg font-bold text-gray-900">
              {isPossible ? "의료기기 수입·유통허가 진행이 가능합니다" : "보완이 필요할 수 있습니다"}
            </p>

            <p className="mt-2 text-sm leading-relaxed text-gray-600">
              {isPossible
                ? "현재 사업자등록·보관유통시설 준비 상태 기준으로 의료기기 수입·유통허가 신청 요건을 충족합니다."
                : "현재 사업자등록 또는 보관유통시설 준비 상태만으로는 의료기기 허가 신청이 자동으로 진행되지 않습니다. 제품 분류와 품질서류를 보완하면 진행할 수 있는 경우가 많습니다."}
            </p>
          </div>

          <MedicalDeviceScoreGauge score={diagnosis.feasibilityScore} tone={diagnosis.resultTone} />
        </div>
...
export default function RegisterMedicalDevicePage() {
  ...
  const isUnlicensedOperating = medicalDeviceChoice === "distributing_unlicensed";
  ...
  const result: ResultTone =
    registrationStatus && facilityStatus
      ? registrationStatus === "confirmed" && facilityStatus === "secured"
        ...
        : "conditional"
      : null;
  const showResult = Boolean(medicalDeviceChoice && !isUnlicensedOperating && registrationStatus && facilityStatus && qualityDocStatus);
  ...
  const requiredDocs = getRequiredDocuments("register_medical_device");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const created = await submitMemberRegisterLead({
        serviceType: "register_medical_device",
        sourcePage: "/register/medical-device",
        result,
        diagnosisAction: "register_medical_device_diagnosis_lead",
    ...
    diagnosis,
    result,
    medicalDeviceChoice, registrationStatus, facilityStatus, qualityDocStatus,
    previousRejection,
    rejectionReason,
    lang,
  ...
  function recordRejectionAnonymously() {
    ...
    pendingRejectionInsertRef.current = supabase
      ...
      .insert({
        id,
        service_type: "register_medical_device",
  ...
  async function handleLeadSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    await supabase.from("crm_activities").insert({
      lead_id: newLeadId,
      action: "register_medical_device_diagnosis_lead",
      tag: "REGISTER_MEDICAL_DEVICE",
      meta: diagnosis
        ? {
            feasibilityScore: diagnosis.feasibilityScore,
    ...
    >
      <FunnelPageHeader
        ...
        description={
          !costEntryDone
            ? contextTab === "review"
              ? "현재 받은 안내나 견적이 의료기기 수입·유통허가에 필요한 절차와 비용 기준에 맞는지 확인합니다."
              : contextTab === "direct"
                ? "의료기기 수입·유통허가의 기본 절차·준비 항목·추가 절차 가능성을 확인합니다."
      ...
      />
            ...
            <VerifyStepLayout
              ...
              question={
                <>
                  <QuestionSection
                    step={1}
                    title="이전에 다른 곳(정부기관 또는 타 대행사)에서 의료기기 수입·유통허가를 신청하셨다가 거절·반려되신 적이 있나요?"
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && !medicalDeviceChoice && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="register"
              step={2}
              question={
                <QuestionSection
                  step={2}
                  title="현재 의료기기를 어떻게 수입·유통하고 계신가요?"
                    ...
                    {OPERATION_OPTIONS.map((opt) => (
                      <SelectionCard
                        ...
                        onClick={() => {
                          ...
                          setTimeout(() => {
                            setMedicalDeviceChoice(opt.key);
                            setSelectedKey(null);
                          }, 300);
                        }}
                      />
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && medicalDeviceChoice && !isUnlicensedOperating && !registrationStatus && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="register"
              step={3}
              question={
                <QuestionSection
                  ...
                  description="의료기기 허가 신청에 앞서 확인하는 기본 등록 서류입니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && registrationStatus && facilityStatus && !qualityDocStatus && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="register"
              step={4}
              question={
                <QuestionSection
                  step={5}
                  title="제품 분류와 품질서류를 준비하셨나요?"
                  description="의료기기 허가 관련 분류·품질서류 준비 상태를 확인합니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
        ...
        {!restoreRegisterPending && costEntryDone && canShowResults && diagnosis && leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-5 sm:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            <p className="text-[11px] font-semibold uppercase tracking-widest text-gray-400">
              의료기기 수입·유통허가 · AI 분석 리포트
            </p>

            <MedicalDeviceDesktopResultHeader diagnosis={diagnosis} />

            <MedicalDeviceResultOverviewCards diagnosis={diagnosis} docCount={requiredDocs.documents.length} />

            <MedicalDeviceResultSummaryCard diagnosis={diagnosis} />
            ...
            <p className="mt-2 text-[11px] text-gray-400">
              공공서비스포털(Cổng Dịch vụ công quốc gia)의 의료기기 수입·유통허가 절차 안내
