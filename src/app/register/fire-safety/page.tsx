...
export default function RegisterFireSafetyPage() {
  ...
  async function handleLeadSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const res = await fetch("/api/lead-submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ leadId: newLeadId, name, phone, phoneDigits, email, address, lang, kakao_id: kakaoId, zalo_id: zaloId }),
      });
      if (!res.ok) {
  ...
  return (
    <main className="min-h-screen bg-[#fafafa]">
      ...
      <div className={`mx-auto px-6 py-10 ${resultScreenActive ? "max-w-4xl" : "max-w-xl"}`}>
        ...
        {rejectionStepDone && !fireSafetyChoice && (
          <div className="mt-8">
            <QuestionSection step={2} title="현재 영업장을 어떻게 운영하고 계신가요?">
        ...
        {/* 질문 3 — 사업자·법인 등록 서류 준비 (모바일 2열 유지 — FireSafety 확정 요구사항) */}
        {rejectionStepDone && fireSafetyChoice && !isUnlicensedOperating && !registrationStatus && (
          <div className="mt-8">
            <QuestionSection step={3} title="사업자·법인(영업장) 등록 서류가 준비되어 있나요?">
              <div className="grid grid-cols-2 gap-3">
        ...
        {rejectionStepDone && registrationStatus && !facilityStatus && (
          <div className="mt-8">
            <QuestionSection step={4} title="소방시설을 설치하셨나요?">
              <div className="grid grid-cols-2 gap-3">
        ...
        {rejectionStepDone && registrationStatus && facilityStatus && !safetyManagerStatus && (
          <div className="mt-8">
            <QuestionSection step={5} title="소방안전관리자 선임과 소방계획서 준비를 마치셨나요?">
              <div className="grid grid-cols-2 gap-3">
