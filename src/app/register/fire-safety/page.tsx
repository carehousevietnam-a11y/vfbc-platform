...
// 소방허가(소방시설 완비증명)는 관할 지역·건축물 용도에 따라 담당부서가
// 달라, 이 포털에서 관할 지역을 선택해 안내를 받도록 연결한다. (특정 부서
// URL을 직접 지정하지 않음)
...
// 자체 진단 로직 (checkDiagnosis.ts 미사용, 규칙 기반) — 등록상태·소방시설
// 설치 여부로 점수를 계산하고, 소방안전관리자·소방계획서 준비상태는 체크리스트
// 항목으로만 반영한다. 법 조항·구체적 허가가능 여부는 단정하지 않고 "가능성"
// 톤을 유지한다.
...
function computeFireSafetyDiagnosis(
  ...
  const checklist = [
    { label: "사업자·법인(영업장) 등록 서류 준비", passed: registrationStatus === "confirmed" },
  ...
  return {
    ...
    checklist,
    note,
    estimatedDays: { min: 10, max: 25 },
  };
}

function DiagnosisReportCard({ diagnosis }: { diagnosis: FireSafetyDiagnosis }) {
...
export default function RegisterFireSafetyPage() {
  ...
  const showResult = Boolean(
    fireSafetyChoice && !isUnlicensedOperating && registrationStatus && facilityStatus && safetyManagerStatus
  );
  ...
  function recordRejectionAnonymously() {
    ...
    pendingRejectionInsertRef.current = supabase
      ...
      .insert({
        id,
        service_type: "register_fire-safety",
  ...
  async function handleLeadSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    await supabase.from("crm_activities").insert({
      lead_id: newLeadId,
      action: "register_fire-safety_diagnosis_lead",
      tag: "REGISTER_FIRE",
      meta: diagnosis
        ? {
    ...
    <main className="min-h-screen bg-[#fafafa]">
      ...
      <div className="mx-auto max-w-xl px-6 py-10">
        ...
        {!rejectionStepDone && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              1. 이전에 다른 곳(정부기관 또는 타 대행사)에서 소방허가(소방시설
              완비증명)를 신청하셨다가 거절·반려되신 적이 있나요?
            </p>
        ...
        {rejectionStepDone && !fireSafetyChoice && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              2. 현재 영업장을 어떻게 운영하고 계신가요?
            </p>
            <div className="mt-4 grid gap-3 sm:grid-cols-1">
              {[
                { key: "not_started", label: "아직 운영 전입니다", desc: "허가 신청을 준비하는 단계" },
                { key: "operating_licensed", label: "완비증명을 받고 정상 운영 중입니다", desc: "" },
                { key: "operating_unlicensed", label: "완비증명 없이 이미 운영 중입니다", desc: "" },
              ].map((opt) => (
                <button
                  key={opt.key}
                  onClick={() => setFireSafetyChoice(opt.key as FireSafetyChoice)}
        ...
        {rejectionStepDone && isUnlicensedOperating && (
          <div className="mt-8 rounded-3xl bg-white border border-red-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-4 text-lg font-bold text-gray-900">
              미필증 상태는 영업정지 대상이 될 수 있습니다
            </p>
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              소방시설 완비증명 없이 영업 중인 경우 단속 시 즉시 영업정지
              조치될 수 있으며, 이후 정식 신청에도 불이익이 있을 수
              있습니다. 가능한 빨리 완비증명 절차를 진행하시길 권합니다.
            </p>
            <div className="mt-5 flex flex-col gap-3">
              <button
                onClick={() => setFireSafetyChoice(null)}
        ...
        {rejectionStepDone && registrationStatus && !facilityStatus && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              4. 소방시설(소화기·경보설비·스프링클러 등)을 설치하셨나요?
            </p>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {[
                { key: "secured", label: "설치 완료" },
                { key: "unsecured", label: "아직 미설치" },
              ].map((opt) => (
                <button
                  key={opt.key}
          ...
          <div className="mt-8">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {[
                ...
                <button
                  key={opt.key}
                  onClick={() => setSafetyManagerStatus(opt.key as SafetyManagerStatus)}
                  className="rounded-2xl bg-white border border-gray-100 p-4 text-sm font-semibold text-gray-900 shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:-translate-y-0.5 transition-all"
                >
                  {opt.label}
                </button>
        ...
        {showResult && result === "possible" && !leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              현재 사업자등록·소방시설 준비 상태 기준으로 소방시설 완비증명
              신청 요건을 충족합니다.
            </p>
        ...
        {showResult && result === "possible" && leadSubmitted && !agencyRequested && !detailStage && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <div className="mt-4 rounded-xl bg-gray-50 px-4 py-3">
              <p className="text-xs font-semibold text-gray-700">소방허가에 필요한 서류</p>
              ...
              <p className="mt-2 text-[11px] text-gray-400">
                정확한 요건은 건축물 용도·규모에 따라 다를 수 있어 담당자
                확인이 필요합니다.
              </p>
            </div>
            ...
            <div className="mt-3 flex flex-col gap-3">
              <a
                href={REGISTER_FIRE_SAFETY_OFFICIAL_URL}
                target="_blank"
            ...
            <p className="mt-2 text-[11px] text-gray-400">
              ...
              접속 후 관할 지역과 소방 담당부서를 선택하시면 신청 메뉴를
              찾으실 수 있습니다.
            </p>
        ...
        {/* 7. 진행 서류 및 절차 안내 (전문가 대행 선택 시) */}
        {showResult && result === "possible" && leadSubmitted && !agencyRequested && detailStage && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            <CheckCircle2 className="text-emerald-600" size={28} />
            <p className="mt-4 text-lg font-bold text-gray-900">소방허가 진행 서류 및 절차</p>
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="rounded-xl bg-gray-50 px-4 py-3">
                <p className="text-xs font-semibold text-gray-700">② 시설·관리자 준비</p>
                <ul className="mt-2 space-y-2">
                  <li className="text-xs text-gray-600 pl-1">
                    · 소방시설(소화기·경보설비·스프링클러 등) 설치 확인
                  </li>
                  <li className="text-xs text-gray-600 pl-1">
                    · 소방안전관리자 선임 및 소방계획서 작성
                  </li>
                  {safetyManagerStatus === "not_ready" && (
                    <li className="text-xs text-gray-600 pl-1">
                      · 소방안전관리자·소방계획서가 아직 준비되지 않아,
                      추가 준비 후 재검토가 필요할 수 있습니다 — 이 부분은
                      대행 접수 시 담당자가 우선 확인해드립니다.
                    </li>
                  )}
                </ul>
              </div>
              ...
              <div className="rounded-xl bg-gray-50 px-4 py-3">
                ...
                <p className="mt-2 text-xs text-gray-600 leading-relaxed">
                  서류 준비 → 소방시설 설치 확정 → 소방완비증명 신청·접수
                  → 현장점검 → 완비증명 발급 → 영업 개시
                </p>
                <p className="mt-2 text-[11px] text-gray-400">
                  * 위 절차는 일반적인 흐름 안내이며, 건축물 용도·규모에
        ...
        {showResult && result === "conditional" && !leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-amber-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              현재 사업자등록 또는 소방시설 준비 상태만으로는 소방허가
        ...
        {showResult && result === "conditional" && leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-amber-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <Link
              href="/consultation?case=register-fire-safety-conditional"
