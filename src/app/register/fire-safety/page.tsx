...
// 소방허가는 관할 지역(성·시)에 따라 담당부서가 달라, 이 포털에서
// 관할 지역을 선택해 안내를 받도록 연결한다. (특정 부서 URL을 직접 지정하지 않음)
// ⚠️ 배포 전 Linda 법률 검토 필요 — URL·안내 문구 확인 후 게시할 것.
const REGISTER_FIRE_SAFETY_OFFICIAL_URL = "https://dichvucong.gov.vn/";

const REGISTER_QUESTION_CONTEXT = "소방허가 가능성 진단";
...
type SafetyManagerStatus = "ready" | "not_ready" | null;
type FireSafetyChoice = "not_started" | "operating_licensed" | "operating_unlicensed" | null;
...
const OPERATION_OPTIONS: { key: NonNullable<FireSafetyChoice>; label: string; desc: string; icon: typeof Store; tone: SelectionCardTone }[] = [
  { key: "not_started", label: "아직 영업 전입니다", desc: "소방허가 신청을 준비하는 단계입니다.", icon: Store, tone: "blue" },
  { key: "operating_licensed", label: "허가를 받고 정상 영업 중입니다", desc: "현재 허가 상태를 다시 확인합니다.", icon: CheckCircle2, tone: "green" },
  { key: "operating_unlicensed", label: "허가 없이 이미 영업 중입니다", desc: "무허가 영업 위험을 우선 확인합니다.", icon: AlertTriangle, tone: "red" },
];
...
const PREMISES_OPTIONS: { key: NonNullable<FacilityStatus>; label: string; desc: string; icon: typeof Store; tone: SelectionCardTone }[] = [
  { key: "secured", label: "설치 완료", desc: "소화기·경보·스프링클러 등 소방시설 설치를 완료했습니다.", icon: Store, tone: "green" },
  { key: "unsecured", label: "아직 미설치", desc: "필요한 소방시설 설치가 남아 있습니다.", icon: FileWarning, tone: "amber" },
];

const HYGIENE_OPTIONS: { key: NonNullable<SafetyManagerStatus>; label: string; desc: string; icon: typeof Flame; tone: SelectionCardTone }[] = [
  { key: "ready", label: "예, 완료했습니다", desc: "소방안전관리자 선임과 소방계획서 준비를 완료했습니다.", icon: CheckCircle2, tone: "green" },
  { key: "not_ready", label: "아직입니다", desc: "소방안전관리자 또는 소방계획서 준비가 남아 있습니다.", icon: Flame, tone: "amber" },
];
...
function computeFireSafetyDiagnosis(
  ...
  const checklist = [
    ...
    { label: "소방시설(소화기·경보·스프링클러 등) 설치 확보", passed: facilityStatus === "secured" },
    { label: "소방안전관리자 선임 및 소방계획서 준비", passed: safetyManagerStatus === "ready" },
  ];
...
// 확인하여 구조·className·순서를 옮겨왔다. 값만 REGISTER(fire-safety) 진단 결과로 채운다.

// TRC의 PremiumLeadCapture 안 원형 게이지와 100% 동일한 마크업 — 결과 미리보기(가입 전)용.
function FireSafetyScoreGauge({
  score,
  tone,
}: {
  score: number;
...
// 추가하지 않는다.
function FireSafetyResultOverviewCards({
  diagnosis,
  docCount,
}: {
  diagnosis: FireSafetyDiagnosis;
  ...
  const items = [
    {
      n: 1,
      label: "허가 가능성",
      visual: <FireSafetyScoreGauge score={feasibilityScore} tone={resultTone} />,
...
function FireSafetyDesktopResultHeader({
  diagnosis,
}: {
  diagnosis: FireSafetyDiagnosis;
}) {
...
function FireSafetyResultSummaryCard({ diagnosis }: { diagnosis: FireSafetyDiagnosis }) {
...
// 길이·모바일/PC 배열을 그대로 일치시킨 3버튼 CTA. 서비스명(소방허가)과 연결
// 대상(공식 사이트 URL, fire-safety CRM)만 FireSafety에 맞게 대체했다.
...
function FireSafetyNextStepOptions({
  onSelf,
  onExpert,
  onAiReport,
  officialUrl,
...
function FireSafetyLeadCapture({
  diagnosis,
  messengers,
  lang,
  fieldErrors,
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-4 text-lg font-bold text-gray-900">
              {isPossible ? "소방허가 진행이 가능합니다" : "보완이 필요할 수 있습니다"}
            </p>

            <p className="mt-2 text-sm leading-relaxed text-gray-600">
              {isPossible
                ? "현재 사업자등록·소방시설 준비 상태 기준으로 소방허가 신청 요건을 충족합니다."
                : "현재 사업자등록 또는 소방시설 준비 상태만으로는 소방허가 신청이 자동으로 진행되지 않습니다. 준비 서류와 시설을 보완하면 진행할 수 있는 경우가 많습니다."}
            </p>
          </div>

          <FireSafetyScoreGauge score={diagnosis.feasibilityScore} tone={diagnosis.resultTone} />
        </div>
...
export default function RegisterFireSafetyPage() {
  const [fireSafetyChoice, setFireSafetyChoice] = useState<FireSafetyChoice>(null);
  ...
  const [safetyManagerStatus, setSafetyManagerStatus] = useState<SafetyManagerStatus>(null);
  ...
  function applyRestoredRegister(restored: RestoredRegisterLead) {
    ...
    if (meta) {
      if (meta.fireSafetyChoice) setFireSafetyChoice(meta.fireSafetyChoice as FireSafetyChoice);
      ...
      if (meta.safetyManagerStatus) setSafetyManagerStatus(meta.safetyManagerStatus as SafetyManagerStatus);
  ...
  async function handleLandingContinue() {
    const { loggedIn, restored } = await loadRegisterMemberEntryState(
      ...
      "register_fire_safety_diagnosis_lead",
      { allowRestore: true }
    );
  ...
  const isUnlicensedOperating = fireSafetyChoice === "operating_unlicensed";
  ...
  const result: ResultTone =
    registrationStatus && facilityStatus
      ? registrationStatus === "confirmed" && facilityStatus === "secured"
        ...
        : "conditional"
      : null;
  const showResult = Boolean(fireSafetyChoice && !isUnlicensedOperating && registrationStatus && facilityStatus && safetyManagerStatus);
  ...
  const diagnosis = canShowResults
    ? computeFireSafetyDiagnosis(registrationStatus, facilityStatus, safetyManagerStatus)
    : null;

  const requiredDocs = getRequiredDocuments("register_fire_safety");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const created = await submitMemberRegisterLead({
        serviceType: "register_fire_safety",
        sourcePage: "/register/fire-safety",
        result,
        diagnosisAction: "register_fire_safety_diagnosis_lead",
        tag: "REGISTER_FIRE_SAFETY",
        meta: {
    ...
    diagnosis,
    result,
    fireSafetyChoice, registrationStatus, facilityStatus, safetyManagerStatus,
    previousRejection,
    rejectionReason,
    lang,
  ...
  function recordRejectionAnonymously() {
    ...
    pendingRejectionInsertRef.current = supabase
      ...
      .insert({
        id,
        service_type: "register_fire_safety",
        source_page: "/register/fire-safety",
        reason: null,
      })
      .then(({ error }) => {
        if (error) {
  ...
  function reset() {
    setFireSafetyChoice(null);
    setRegistrationStatus(null);
    setFacilityStatus(null);
    setSafetyManagerStatus(null);
  ...
  async function handleExpertRequest() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=register_fire_safety&mode=expert`;
        return;
      }
  ...
  async function handleAiReportRequest() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=register_fire_safety&mode=ai_report`;
        return;
      }
  ...
  async function handleLeadSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    await supabase.from("crm_activities").insert({
      lead_id: newLeadId,
      action: "register_fire_safety_diagnosis_lead",
      tag: "REGISTER_FIRE_SAFETY",
      meta: diagnosis
        ? {
            feasibilityScore: diagnosis.feasibilityScore,
    ...
    >
      <FunnelPageHeader
        ...
        description={
          !costEntryDone
            ? contextTab === "review"
              ? "현재 받은 안내나 견적이 소방허가에 필요한 절차와 비용 기준에 맞는지 확인합니다."
              : contextTab === "direct"
                ? "소방허가의 기본 절차·준비 항목·추가 절차 가능성을 확인합니다."
      ...
      />
            ...
            <VerifyStepLayout
              ...
              question={
                <>
                  <QuestionSection
                    step={1}
                    title="이전에 다른 곳(정부기관 또는 타 대행사)에서 소방허가를 신청하셨다가 거절·반려되신 적이 있나요?"
            ...
            {previousRejection === true && (
              <div className="mt-4">
                ...
                <textarea
                  ...
                  placeholder={
                    "예)\n- 소방시설이 기준에 맞지 않는다고 들었습니다.\n- 소방안전관리자 또는 계획서가 부족하다고 들었습니다.\n- 정확한 이유를 듣지 못했습니다.\n\n자유롭게 작성해주세요(선택)."
                  }
                  rows={6}
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && !fireSafetyChoice && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="register"
              step={2}
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && isUnlicensedOperating && (
          <div className={`mt-8 ${FUNNEL_QUESTION_COLUMN}`}>
            ...
            <div className="mt-5 flex flex-col gap-3">
              ...
              <Link
                href="/consultation?case=register-fire-safety-unlicensed-warning"
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && fireSafetyChoice && !isUnlicensedOperating && !registrationStatus && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="register"
              step={3}
              question={
                <QuestionSection
                  step={3}
                  title="사업자·법인(영업장) 등록 서류가 준비되어 있나요?"
                  description="소방허가 신청에 앞서 확인하는 기본 등록 서류입니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
            ...
            <VerifyStepLayout
              ...
              question={
                <QuestionSection
                  step={4}
                  title="소방시설을 설치하셨나요?"
                  description="허가 신청 시 확인할 소방시설 준비 상태입니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && registrationStatus && facilityStatus && !safetyManagerStatus && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="register"
              step={4}
              question={
                <QuestionSection
                  step={5}
                  title="소방안전관리자 선임과 소방계획서 준비를 마치셨나요?"
                  description="소방허가 관련 안전관리·계획서 준비 상태를 확인합니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
                    {HYGIENE_OPTIONS.map((opt) => (
                      <SelectionCard
                        ...
                        onClick={() => {
                          setSelectedKey(opt.key);
                          setTimeout(() => {
                            setSafetyManagerStatus(opt.key);
                            setSelectedKey(null);
                          }, 300);
                        }}
                      />
        ...
        {!restoreRegisterPending && costEntryDone && canShowResults && diagnosis && leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-5 sm:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <FireSafetyDesktopResultHeader diagnosis={diagnosis} />

            <FireSafetyResultOverviewCards diagnosis={diagnosis} docCount={requiredDocs.documents.length} />

            <FireSafetyResultSummaryCard diagnosis={diagnosis} />
            ...
            <p className="mt-2 text-[11px] text-gray-400">
              공공서비스포털(Cổng Dịch vụ công quốc gia)의 소방허가 절차 안내
