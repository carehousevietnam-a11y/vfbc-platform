...
type StaffHygieneStatus = "ready" | "not_ready" | null;
type HygieneChoice = "not_started" | "operating_licensed" | "operating_unlicensed" | null;
...
const OPERATION_OPTIONS: { key: NonNullable<HygieneChoice>; label: string; desc: string; icon: typeof Store; tone: SelectionCardTone }[] = [
  { key: "not_started", label: "아직 영업 전입니다", desc: "위생허가 신청을 준비하는 단계입니다.", icon: Store, tone: "blue" },
...
const PREMISES_OPTIONS: { key: NonNullable<FacilityStatus>; label: string; desc: string; icon: typeof Store; tone: SelectionCardTone }[] = [
  { key: "secured", label: "구비 완료", desc: "조리·저장·세척 시설 등 위생시설을 갖췄습니다.", icon: Store, tone: "green" },
  { key: "unsecured", label: "아직 미구비", desc: "필요한 위생시설 준비가 남아 있습니다.", icon: FileWarning, tone: "amber" },
];

const HYGIENE_OPTIONS: { key: NonNullable<StaffHygieneStatus>; label: string; desc: string; icon: typeof Flame; tone: SelectionCardTone }[] = [
  { key: "ready", label: "예, 완료했습니다", desc: "종사자 건강검진과 위생교육 준비를 완료했습니다.", icon: CheckCircle2, tone: "green" },
  { key: "not_ready", label: "아직입니다", desc: "건강검진 또는 위생교육 준비가 남아 있습니다.", icon: Flame, tone: "amber" },
];
...
function computeHygieneDiagnosis(
  registrationStatus: RegistrationStatus,
  facilityStatus: FacilityStatus,
  staffHygieneStatus: StaffHygieneStatus
): HygieneDiagnosis {
  const checklist = [
    { label: "사업자·법인 등록 서류 준비", passed: registrationStatus === "confirmed" },
    { label: "위생시설(조리·저장·세척 시설 등) 구비 확보", passed: facilityStatus === "secured" },
    { label: "종사자 건강검진·위생교육 이수 준비", passed: staffHygieneStatus === "ready" },
  ];
...
// 확인하여 구조·className·순서를 옮겨왔다. 값만 REGISTER(hygiene) 진단 결과로 채운다.

// TRC의 PremiumLeadCapture 안 원형 게이지와 100% 동일한 마크업 — 결과 미리보기(가입 전)용.
function HygieneScoreGauge({
  score,
  tone,
}: {
  score: number;
...
// 추가하지 않는다.
function HygieneResultOverviewCards({
  diagnosis,
  docCount,
}: {
  diagnosis: HygieneDiagnosis;
  ...
  const items = [
    {
      n: 1,
      label: "허가 가능성",
      visual: <HygieneScoreGauge score={feasibilityScore} tone={resultTone} />,
...
function HygieneDesktopResultHeader({
  diagnosis,
}: {
  diagnosis: HygieneDiagnosis;
}) {
...
function HygieneResultSummaryCard({ diagnosis }: { diagnosis: HygieneDiagnosis }) {
...
// 길이·모바일/PC 배열을 그대로 일치시킨 3버튼 CTA. 서비스명(위생허가)과 연결
// 대상(공식 사이트 URL, hygiene CRM)만 Hygiene에 맞게 대체했다.
...
function HygieneNextStepOptions({
  onSelf,
  onExpert,
  onAiReport,
  officialUrl,
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-4 text-lg font-bold text-gray-900">
              {isPossible ? "위생허가 진행이 가능합니다" : "보완이 필요할 수 있습니다"}
            </p>

            <p className="mt-2 text-sm leading-relaxed text-gray-600">
              {isPossible
                ? "현재 사업자등록·위생시설 준비 상태 기준으로 위생허가 신청 요건을 충족합니다."
                : "현재 사업자등록 또는 위생시설 준비 상태만으로는 위생허가 신청이 자동으로 진행되지 않습니다. 시설과 서류를 보완하면 진행할 수 있는 경우가 많습니다."}
            </p>
          </div>

          <HygieneScoreGauge score={diagnosis.feasibilityScore} tone={diagnosis.resultTone} />
        </div>
...
export default function RegisterHygienePage() {
  const [hygieneChoice, setHygieneChoice] = useState<HygieneChoice>(null);
  ...
  const [staffHygieneStatus, setStaffHygieneStatus] = useState<StaffHygieneStatus>(null);
  ...
  function applyRestoredRegister(restored: RestoredRegisterLead) {
    ...
    if (meta) {
      if (meta.hygieneChoice) setHygieneChoice(meta.hygieneChoice as HygieneChoice);
      ...
      if (meta.staffHygieneStatus) setStaffHygieneStatus(meta.staffHygieneStatus as StaffHygieneStatus);
  ...
  const isUnlicensedOperating = hygieneChoice === "operating_unlicensed";
  ...
  const result: ResultTone =
    registrationStatus && facilityStatus
      ? registrationStatus === "confirmed" && facilityStatus === "secured"
        ...
        : "conditional"
      : null;
  const showResult = Boolean(hygieneChoice && !isUnlicensedOperating && registrationStatus && facilityStatus && staffHygieneStatus);
  ...
  const diagnosis = canShowResults
    ? computeHygieneDiagnosis(registrationStatus, facilityStatus, staffHygieneStatus)
    : null;

  const requiredDocs = getRequiredDocuments("register_hygiene");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const created = await submitMemberRegisterLead({
        serviceType: "register_hygiene",
        sourcePage: "/register/hygiene",
        result,
        diagnosisAction: "register_hygiene_diagnosis_lead",
    ...
    diagnosis,
    result,
    hygieneChoice, registrationStatus, facilityStatus, staffHygieneStatus,
    previousRejection,
    rejectionReason,
    lang,
  ...
  function recordRejectionAnonymously() {
    ...
    pendingRejectionInsertRef.current = supabase
      ...
      .insert({
        id,
        service_type: "register_hygiene",
        source_page: "/register/hygiene",
        reason: null,
      })
      .then(({ error }) => {
        if (error) {
  ...
  function reset() {
    setHygieneChoice(null);
    setRegistrationStatus(null);
    setFacilityStatus(null);
    setStaffHygieneStatus(null);
  ...
  async function handleExpertRequest() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=register_hygiene&mode=expert`;
        return;
      }
  ...
  async function handleAiReportRequest() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=register_hygiene&mode=ai_report`;
        return;
      }
  ...
  async function handleLeadSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    await supabase.from("crm_activities").insert({
      lead_id: newLeadId,
      action: "register_hygiene_diagnosis_lead",
      tag: "REGISTER_HYGIENE",
      meta: diagnosis
        ? {
            feasibilityScore: diagnosis.feasibilityScore,
  ...
  return (
    ...
    >
      ...
      />
            ...
            <VerifyStepLayout
              ...
              question={
                <>
                  <QuestionSection
                    step={1}
                    title="이전에 다른 곳(정부기관 또는 타 대행사)에서 위생허가를 신청하셨다가 거절·반려되신 적이 있나요?"
            ...
            {previousRejection === true && (
              <div className="mt-4">
                ...
                <textarea
                  ...
                  placeholder={
                    "예)\n- 위생시설이 기준에 맞지 않는다고 들었습니다.\n- 종사자 건강검진 또는 위생교육 서류가 부족하다고 들었습니다.\n- 정확한 이유를 듣지 못했습니다.\n\n자유롭게 작성해주세요(선택)."
                  }
                  rows={6}
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && !hygieneChoice && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="register"
              step={2}
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && isUnlicensedOperating && (
          <div className={`mt-8 ${FUNNEL_QUESTION_COLUMN}`}>
            ...
            <div className="mt-5 flex flex-col gap-3">
              ...
              <Link
                href="/consultation?case=register-hygiene-unlicensed-warning"
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && hygieneChoice && !isUnlicensedOperating && !registrationStatus && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="register"
              step={3}
              question={
                <QuestionSection
                  ...
                  description="위생허가 신청에 앞서 확인하는 기본 등록 서류입니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
            ...
            <VerifyStepLayout
              ...
              question={
                <QuestionSection
                  step={4}
                  title="위생시설을 갖추셨나요?"
                  description="허가 신청 시 확인할 위생시설 준비 상태입니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && registrationStatus && facilityStatus && !staffHygieneStatus && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="register"
              step={4}
              question={
                <QuestionSection
                  step={5}
                  title="종사자 건강검진과 위생교육 준비를 마치셨나요?"
                  description="위생허가 관련 종사자 건강·교육 준비 상태를 확인합니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
        ...
        {!restoreRegisterPending && costEntryDone && canShowResults && diagnosis && leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-5 sm:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <HygieneDesktopResultHeader diagnosis={diagnosis} />

            <HygieneResultOverviewCards diagnosis={diagnosis} docCount={requiredDocs.documents.length} />

            <HygieneResultSummaryCard diagnosis={diagnosis} />
            ...
            <p className="mt-2 text-[11px] text-gray-400">
              공공서비스포털(Cổng Dịch vụ công quốc gia)의 위생허가 절차 안내
