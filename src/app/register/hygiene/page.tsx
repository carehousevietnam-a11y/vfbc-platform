...
// 위생허가(식품위생안전인증)는 관할 지역·업종에 따라 담당부서가 달라, 이
// 포털에서 관할 지역을 선택해 안내를 받도록 연결한다. (특정 부서 URL을
// 직접 지정하지 않음)
...
// 자체 진단 로직 (checkDiagnosis.ts 미사용, 규칙 기반) — 등록상태·위생시설
// 구비 여부로 점수를 계산하고, 종사자 건강검진·위생교육 준비상태는 체크리스트
...
function DiagnosisReportCard({ diagnosis }: { diagnosis: HygieneDiagnosis }) {
...
export default function RegisterHygienePage() {
  ...
  const showResult = Boolean(
    hygieneChoice && !isUnlicensedOperating && registrationStatus && facilityStatus && staffHygieneStatus
  );
  ...
  // environment/fire-safety page.tsx와 동일한 패턴
    ...
    <main className="min-h-screen bg-[#fafafa]">
      ...
      <div className="mx-auto max-w-xl px-6 py-10">
        ...
        {!rejectionStepDone && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              1. 이전에 다른 곳(정부기관 또는 타 대행사)에서 위생허가(식품위생
              안전인증)를 신청하셨다가 거절·반려되신 적이 있나요?
            </p>
          ...
          <div className="mt-8">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-1">
              {[
                ...
                { key: "operating_licensed", label: "인증을 받고 정상 운영 중입니다", desc: "" },
                { key: "operating_unlicensed", label: "인증 없이 이미 운영 중입니다", desc: "" },
              ].map((opt) => (
                <button
                  key={opt.key}
                  onClick={() => setHygieneChoice(opt.key as HygieneChoice)}
        ...
        {rejectionStepDone && isUnlicensedOperating && (
          <div className="mt-8 rounded-3xl bg-white border border-red-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-4 text-lg font-bold text-gray-900">
              무인증 상태는 단속 1순위 대상입니다
            </p>
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              위생인증 없이 영업 중인 경우 단속 시 즉시 영업정지 조치될
              수 있으며, 이후 정식 신청에도 불이익이 있을 수 있습니다.
              가능한 빨리 인증 절차를 진행하시길 권합니다.
            </p>
            <div className="mt-5 flex flex-col gap-3">
              <button
                onClick={() => setHygieneChoice(null)}
        ...
        {rejectionStepDone && registrationStatus && !facilityStatus && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              4. 위생시설(조리·저장·세척 시설 등)을 구비하셨나요?
            </p>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {[
                { key: "secured", label: "구비 완료" },
                { key: "unsecured", label: "아직 미구비" },
              ].map((opt) => (
                <button
                  key={opt.key}
        ...
        {rejectionStepDone && registrationStatus && facilityStatus && !staffHygieneStatus && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              5. 종사자 건강검진 및 위생교육을 이수하셨나요?
            </p>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {[
                { key: "ready", label: "예, 이수했습니다" },
                { key: "not_ready", label: "아직입니다" },
              ].map((opt) => (
                <button
                  key={opt.key}
                  onClick={() => setStaffHygieneStatus(opt.key as StaffHygieneStatus)}
                  className="rounded-2xl bg-white border border-gray-100 p-4 text-sm font-semibold text-gray-900 shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:-translate-y-0.5 transition-all"
                >
                  {opt.label}
                </button>
        ...
        {showResult && result === "possible" && !leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              현재 사업자등록·위생시설 준비 상태 기준으로 식품위생안전인증
              신청 요건을 충족합니다.
            </p>
        ...
        {showResult && result === "possible" && leadSubmitted && !agencyRequested && !detailStage && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <div className="mt-4 rounded-xl bg-gray-50 px-4 py-3">
              <p className="text-xs font-semibold text-gray-700">위생허가에 필요한 서류</p>
              ...
              <p className="mt-2 text-[11px] text-gray-400">
                정확한 요건은 업종·규모에 따라 다를 수 있어 담당자 확인이
                필요합니다.
              </p>
            </div>
            ...
            <div className="mt-3 flex flex-col gap-3">
              <a
                href={REGISTER_HYGIENE_OFFICIAL_URL}
                target="_blank"
            ...
            <p className="mt-2 text-[11px] text-gray-400">
              ...
              접속 후 관할 지역과 위생 담당부서를 선택하시면 신청 메뉴를
              찾으실 수 있습니다.
            </p>
        ...
        {showResult && result === "possible" && leadSubmitted && !agencyRequested && detailStage && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            <CheckCircle2 className="text-emerald-600" size={28} />
            <p className="mt-4 text-lg font-bold text-gray-900">위생허가 진행 서류 및 절차</p>
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="rounded-xl bg-gray-50 px-4 py-3">
                <p className="text-xs font-semibold text-gray-700">② 시설·인력 준비</p>
                <ul className="mt-2 space-y-2">
                  <li className="text-xs text-gray-600 pl-1">
                    · 위생시설(조리·저장·세척시설 등) 구비 확인
                  </li>
                  <li className="text-xs text-gray-600 pl-1">
                    · 종사자 건강검진 및 위생교육 이수
                  </li>
                  {staffHygieneStatus === "not_ready" && (
                    <li className="text-xs text-gray-600 pl-1">
                      · 종사자 건강검진·위생교육이 아직 완료되지 않아,
                      이수 후 재확인이 필요할 수 있습니다 — 이 부분은 대행
                      접수 시 담당자가 우선 확인해드립니다.
                    </li>
                  )}
                </ul>
              </div>
              ...
              <div className="rounded-xl bg-gray-50 px-4 py-3">
                ...
                <p className="mt-2 text-xs text-gray-600 leading-relaxed">
                  서류 준비 → 위생시설 구비 확정 → 식품위생안전인증 신청·
                  접수 → 현장점검 → 인증서 발급 → 영업 개시
                </p>
                <p className="mt-2 text-[11px] text-gray-400">
                  * 위 절차는 일반적인 흐름 안내이며, 업종·규모에 따라
                  순서나 요건이 달라질 수 있습니다.
                </p>
              </div>
            </div>
        ...
        {showResult && result === "conditional" && !leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-amber-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              현재 사업자등록 또는 위생시설 준비 상태만으로는 위생허가
        ...
        {showResult && result === "conditional" && leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-amber-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <Link
              href="/consultation?case=register-hygiene-conditional"
