...
// 프랜차이즈(가맹사업) 등록은 산업무역부(Bộ Công Thương) 소관으로, 이 포털에서
// 관할 부서를 선택해 안내를 받도록 연결한다. (특정 부서 URL을 직접 지정하지 않음)
...
type OperatingHistoryStatus = "sufficient" | "insufficient" | null;
...
// 자체 진단 로직 (checkDiagnosis.ts 미사용, 규칙 기반) — 등록상태·직영점 운영이력
// 여부로 점수를 계산하고, 계약서·매뉴얼 준비상태는 체크리스트 항목으로만 반영한다.
...
function computeFranchiseDiagnosis(
  ...
  const checklist = [
    { label: "사업자·법인(가맹본부) 등록 서류 준비", passed: registrationStatus === "confirmed" },
    { label: "직영점 운영 이력 확보", passed: operatingHistoryStatus === "sufficient" },
  ...
  const tone: "possible" | "conditional" =
    registrationStatus === "confirmed" && operatingHistoryStatus === "sufficient" ? "possible" : "conditional";
  const note =
    tone === "possible"
  ...
  return {
    ...
    checklist,
    note,
    estimatedDays: { min: 20, max: 45 },
  };
}

function DiagnosisReportCard({ diagnosis }: { diagnosis: FranchiseDiagnosis }) {
...
export default function RegisterFranchisePage() {
  ...
  const isUnregisteredOperating = franchiseChoice === "operating_unregistered";

  const result: ResultTone =
    registrationStatus && operatingHistoryStatus
      ? registrationStatus === "confirmed" && operatingHistoryStatus === "sufficient"
        ? "possible"
        : "conditional"
      : null;
  const showResult = Boolean(
    franchiseChoice && !isUnregisteredOperating && registrationStatus && operatingHistoryStatus && contractManualStatus
  );
  ...
  // "네, 있습니다" 클릭 즉시 익명으로 저장 — company/restaurant/cosmetics page.tsx와 동일한 패턴
    ...
    <main className="min-h-screen bg-[#fafafa]">
      ...
      <div className="mx-auto max-w-xl px-6 py-10">
        ...
        {!rejectionStepDone && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              1. 이전에 다른 곳(정부기관 또는 타 대행사)에서 프랜차이즈(가맹사업)
              등록을 신청하셨다가 거절·반려되신 적이 있나요?
            </p>
        ...
        {rejectionStepDone && !franchiseChoice && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              2. 현재 프랜차이즈(가맹사업)를 어떻게 운영하고 계신가요?
            </p>
            <div className="mt-4 grid gap-3 sm:grid-cols-1">
              {[
                { key: "not_started", label: "아직 가맹사업을 시작하지 않았습니다", desc: "등록 신청을 준비하는 단계" },
                { key: "operating_registered", label: "등록을 마치고 가맹점을 모집·운영 중입니다", desc: "" },
                { key: "operating_unregistered", label: "등록 없이 이미 가맹점을 모집·운영 중입니다", desc: "" },
              ].map((opt) => (
                <button
                  key={opt.key}
                  onClick={() => setFranchiseChoice(opt.key as FranchiseChoice)}
        ...
        {rejectionStepDone && isUnregisteredOperating && (
          <div className="mt-8 rounded-3xl bg-white border border-red-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            <AlertTriangle className="text-red-600" size={28} />
            <p className="mt-4 text-lg font-bold text-gray-900">
              미등록 가맹사업은 계약이 무효화될 수 있습니다
            </p>
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              등록 없이 가맹점을 모집·운영하는 경우 가맹계약 자체가 법적
              효력을 인정받지 못하거나 과태료 등 불이익을 받을 수 있으며,
              이후 정식 등록 신청에도 영향이 있을 수 있습니다. 가능한 빨리
              등록 절차를 진행하시길 권합니다.
            </p>
            <div className="mt-5 flex flex-col gap-3">
              <button
                onClick={() => setFranchiseChoice(null)}
                className="h-12 rounded-full bg-amber-600 text-sm font-semibold text-white hover:bg-amber-700 transition-colors"
              >
                다시 선택하기
              </button>
              <Link
                href="/consultation?case=register-franchise-unregistered-warning"
        ...
        {rejectionStepDone && franchiseChoice && !isUnregisteredOperating && !registrationStatus && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              3. 사업자·법인(가맹본부) 등록 서류가 준비되어 있나요?
            </p>
        ...
        {rejectionStepDone && registrationStatus && !operatingHistoryStatus && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              4. 직영점(본사 운영 매장) 운영 이력을 충분히 확보하셨나요?
            </p>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {[
                { key: "sufficient", label: "충분히 확보" },
                { key: "insufficient", label: "아직 부족" },
              ].map((opt) => (
                <button
                  key={opt.key}
                  onClick={() => setOperatingHistoryStatus(opt.key as OperatingHistoryStatus)}
                  className="rounded-2xl bg-white border border-gray-100 p-4 text-sm font-semibold text-gray-900 shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:-translate-y-0.5 transition-all"
                >
                  {opt.label}
                </button>
          ...
          <div className="mt-8">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {[
                ...
                <button
                  key={opt.key}
                  onClick={() => setContractManualStatus(opt.key as ContractManualStatus)}
                  className="rounded-2xl bg-white border border-gray-100 p-4 text-sm font-semibold text-gray-900 shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:-translate-y-0.5 transition-all"
                >
                  {opt.label}
                </button>
        ...
        {showResult && result === "possible" && !leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              현재 사업자등록·직영점 운영이력 기준으로 가맹사업 등록 신청
              요건을 충족합니다.
            </p>
        ...
        {showResult && result === "possible" && leadSubmitted && !agencyRequested && !detailStage && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <div className="mt-4 rounded-xl bg-gray-50 px-4 py-3">
              <p className="text-xs font-semibold text-gray-700">프랜차이즈허가에 필요한 서류</p>
              <ul className="mt-2 space-y-1">
                ...
                <li className="text-xs text-gray-600 pl-1">
                  · 직영점 운영이력 증빙 자료
                  {operatingHistoryStatus === "insufficient" && (
                    <span className="ml-1 text-amber-700 font-semibold">(준비 필요)</span>
                  )}
                </li>
                <li className="text-xs text-gray-600 pl-1">
                  · 가맹계약서(표준양식) 및 운영매뉴얼
                  {contractManualStatus === "not_ready" && (
                    <span className="ml-1 text-amber-700 font-semibold">(준비 필요)</span>
                  )}
                </li>
                <li className="text-xs text-gray-600 pl-1">
              ...
              <p className="mt-2 text-[11px] text-gray-400">
                정확한 요건은 업종·가맹점 규모에 따라 다를 수 있어 담당자
                확인이 필요합니다.
              </p>
            </div>
            ...
            <div className="mt-3 flex flex-col gap-3">
              <a
                href={REGISTER_FRANCHISE_OFFICIAL_URL}
                target="_blank"
            ...
            <p className="mt-2 text-[11px] text-gray-400">
              ...
              접속 후 관할 부서(산업무역부 소관)를 선택하시면 가맹사업
              등록 신청 메뉴를 찾으실 수 있습니다.
            </p>
        ...
        {showResult && result === "possible" && leadSubmitted && !agencyRequested && detailStage && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            <CheckCircle2 className="text-emerald-600" size={28} />
            <p className="mt-4 text-lg font-bold text-gray-900">프랜차이즈허가 진행 서류 및 절차</p>
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="rounded-xl bg-gray-50 px-4 py-3">
                <p className="text-xs font-semibold text-gray-700">② 계약·매뉴얼 준비</p>
                <ul className="mt-2 space-y-2">
                  <li className="text-xs text-gray-600 pl-1">
                    · 가맹계약서(표준양식) 작성
                  </li>
                  <li className="text-xs text-gray-600 pl-1">
                    · 운영매뉴얼 및 가맹사업 정보공개서 작성
                  </li>
                  {operatingHistoryStatus === "insufficient" && (
                    <li className="text-xs text-gray-600 pl-1">
                      · 직영점 운영이력이 아직 충분하지 않아, 추가 운영기간
                      확보 또는 대체 증빙 검토가 필요할 수 있습니다 — 이
                      부분은 대행 접수 시 담당자가 우선 확인해드립니다.
                    </li>
                  )}
                </ul>
              </div>
              ...
              <div className="rounded-xl bg-gray-50 px-4 py-3">
                ...
                <p className="mt-2 text-xs text-gray-600 leading-relaxed">
                  서류 준비 → 직영점 운영이력 확인 → 가맹계약서·정보공개서
                  등록 신청 → 등록증 발급 → 가맹점 모집 개시
                </p>
                <p className="mt-2 text-[11px] text-gray-400">
                  * 위 절차는 일반적인 흐름 안내이며, 업종·가맹점 규모에
        ...
        {showResult && result === "conditional" && !leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-amber-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              현재 사업자등록 또는 직영점 운영이력만으로는 프랜차이즈허가
        ...
        {showResult && result === "conditional" && leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-amber-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <Link
              href="/consultation?case=register-franchise-conditional"
