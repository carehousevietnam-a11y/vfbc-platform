...
const REGISTER_QUESTION_CONTEXT = "프랜차이즈 등록 가능성 진단";
...
export default function RegisterFranchisePage() {
  ...
  async function handleExpertRequest() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=register_franchise&mode=expert`;
        return;
      }
  ...
  async function handleAiReportRequest() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=register_franchise&mode=ai_report`;
        return;
      }
    ...
    >
      <FunnelPageHeader
        ...
        description={
          !costEntryDone
            ? contextTab === "review"
              ? "현재 받은 안내나 견적이 프랜차이즈 등록에 필요한 절차와 비용 기준에 맞는지 확인합니다."
              : contextTab === "direct"
                ? "프랜차이즈 등록의 기본 절차·준비 항목·추가 절차 가능성을 확인합니다."
          ...
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              ...
              question={
                <QuestionSection
                  step={3}
                  title="사업자·법인(가맹본부) 등록 서류가 준비되어 있나요?"
                  description="프랜차이즈 등록 신청에 앞서 확인하는 기본 등록 서류입니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
          ...
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              ...
              question={
                <QuestionSection
                  step={4}
                  title="직영점 운영 이력을 확보하셨나요?"
                  description="등록 신청 시 확인할 직영점 운영 이력 상태입니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
          ...
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              ...
              question={
                <QuestionSection
                  step={5}
                  title="가맹계약서와 운영매뉴얼을 준비하셨나요?"
                  description="프랜차이즈 등록 관련 계약·매뉴얼 준비 상태를 확인합니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
