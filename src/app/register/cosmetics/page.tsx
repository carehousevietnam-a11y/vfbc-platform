...
// 자체 진단 로직 (checkDiagnosis.ts 미사용, 규칙 기반) — 등록상태·시설확보
// 여부로 점수를 계산하고, 제품 안전성자료 준비상태는 체크리스트 항목으로만 반영한다.
...
function DiagnosisReportCard({ diagnosis }: { diagnosis: CosmeticsDiagnosis }) {
...
export default function RegisterCosmeticsPage() {
  ...
  const showResult = Boolean(
    distributionChoice && !isUnlicensedDistributing && registrationStatus && facilityStatus && safetyDataStatus
  );
  ...
  // "네, 있습니다" 클릭 즉시 익명으로 저장 — company/restaurant page.tsx와 동일한 패턴
    ...
    <main className="min-h-screen bg-[#fafafa]">
      ...
      <div className="mx-auto max-w-xl px-6 py-10">
        ...
        {!rejectionStepDone && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              1. 이전에 다른 곳(정부기관 또는 타 대행사)에서 화장품허가(유통고시)를
        ...
        {rejectionStepDone && !distributionChoice && (
          <div className="mt-8">
            <p className="text-sm font-semibold text-gray-900">
              2. 현재 화장품을 어떻게 유통하고 계신가요?
            </p>
            <div className="mt-4 grid gap-3 sm:grid-cols-1">
              {[
                { key: "not_launched", label: "아직 출시 전입니다", desc: "허가 신청을 준비하는 단계" },
                { key: "distributing_licensed", label: "허가를 받고 정상 유통 중입니다", desc: "" },
                { key: "distributing_unlicensed", label: "허가 없이 이미 유통 중입니다", desc: "" },
              ].map((opt) => (
                <button
                  key={opt.key}
                  onClick={() => setDistributionChoice(opt.key as DistributionChoice)}
        ...
        {rejectionStepDone && isUnlicensedDistributing && (
          <div className="mt-8 rounded-3xl bg-white border border-red-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-4 text-lg font-bold text-gray-900">
              무허가 유통은 전량 회수될 수 있습니다
            </p>
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              유통고시(공고) 없이 판매 중인 경우 적발 시 제품 전량 회수·
              판매중지 조치될 수 있으며, 이후 정식 신청에도 불이익이 있을
              수 있습니다. 가능한 빨리 허가 절차를 진행하시길 권합니다.
            </p>
            <div className="mt-5 flex flex-col gap-3">
              <button
                onClick={() => setDistributionChoice(null)}
          ...
          <div className="mt-8">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              {[
                ...
                <button
                  key={opt.key}
                  onClick={() => setSafetyDataStatus(opt.key as SafetyDataStatus)}
                  className="rounded-2xl bg-white border border-gray-100 p-4 text-sm font-semibold text-gray-900 shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:-translate-y-0.5 transition-all"
                >
                  {opt.label}
                </button>
        ...
        {showResult && result === "possible" && !leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              현재 사업자등록·시설 준비 상태 기준으로 화장품 유통고시(공고)
              신청 요건을 충족합니다.
            </p>
        ...
        {showResult && result === "possible" && leadSubmitted && !agencyRequested && !detailStage && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <div className="mt-4 rounded-xl bg-gray-50 px-4 py-3">
              <p className="text-xs font-semibold text-gray-700">화장품허가에 필요한 서류</p>
              ...
              <p className="mt-2 text-[11px] text-gray-400">
                정확한 요건은 품목·원산지에 따라 다를 수 있어 담당자 확인이
                필요합니다.
              </p>
            </div>
            ...
            <div className="mt-3 flex flex-col gap-3">
              <a
                href={REGISTER_COSMETICS_OFFICIAL_URL}
                target="_blank"
            ...
            <p className="mt-2 text-[11px] text-gray-400">
              ...
              접속 후 관할 지역과 화장품 유통고시 담당부서를 선택하시면
              신청 메뉴를 찾으실 수 있습니다.
            </p>
        ...
        {showResult && result === "possible" && leadSubmitted && !agencyRequested && detailStage && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            <CheckCircle2 className="text-emerald-600" size={28} />
            <p className="mt-4 text-lg font-bold text-gray-900">화장품허가 진행 서류 및 절차</p>
            ...
            <div className="mt-4 space-y-3">
              <div className="rounded-xl bg-gray-50 px-4 py-3">
                ...
                <ul className="mt-2 space-y-2">
                  ...
                  <li className="text-xs text-gray-600 pl-1">
                    · 제조사(해외 제조 시) 위임장 및 제품 성분표
                  </li>
                  <li className="text-xs text-gray-600 pl-1">
                    · 창고·보관 시설 계약서 및 시설 관리 서류
                  </li>
                </ul>
              </div>

              <div className="rounded-xl bg-gray-50 px-4 py-3">
                <p className="text-xs font-semibold text-gray-700">② 제품자료 준비</p>
                <ul className="mt-2 space-y-2">
                  <li className="text-xs text-gray-600 pl-1">
                    · 제품 성분·안전성 평가자료(PIF, Product Information File)
                  </li>
                  <li className="text-xs text-gray-600 pl-1">
                    · 제품 라벨·포장 표기 검토(성분·원산지·사용법 표기 확인)
                  </li>
                  {safetyDataStatus === "not_ready" && (
                    <li className="text-xs text-gray-600 pl-1">
                      · 안전성 평가자료가 아직 준비되지 않아, 자료 보완 후
              ...
              <div className="rounded-xl bg-gray-50 px-4 py-3">
                ...
                <p className="mt-2 text-xs text-gray-600 leading-relaxed">
                  서류 준비 → 창고·보관 시설 확정 → 제품 유통고시(공고)
                  신청·접수 → 라벨·표기 검토 → 고시번호 발급 → 유통 개시
                </p>
                <p className="mt-2 text-[11px] text-gray-400">
                  * 위 절차는 일반적인 흐름 안내이며, 품목·원산지에 따라
                  순서나 요건이 달라질 수 있습니다.
                </p>
              </div>
            </div>
        ...
        {showResult && result === "conditional" && !leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-amber-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <p className="mt-2 text-sm text-gray-600 leading-relaxed">
              현재 사업자등록 또는 시설 준비 상태만으로는 화장품허가(유통고시)
        ...
        {showResult && result === "conditional" && leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-amber-100 p-7 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <Link
              href="/consultation?case=register-cosmetics-conditional"
