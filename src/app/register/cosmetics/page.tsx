...
import {
  ...
  MASTER_LANDING_COSMETICS,
} from "@/components/cost-check/MasterFunnelLanding";

// 베트남 공공서비스포털 (Cổng Dịch vụ công quốc gia).
// 화장품 유통고시(공고)는 관할 지역·품목에 따라 담당부서가 달라, 이 포털에서
// 관할 지역을 선택해 안내를 받도록 연결한다. (특정 부서 URL을 직접 지정하지 않음)
// ⚠️ 배포 전 Linda 법률 검토 필요 — URL·안내 문구 확인 후 게시할 것.
const REGISTER_COSMETICS_OFFICIAL_URL = "https://dichvucong.gov.vn/";

const REGISTER_QUESTION_CONTEXT = "화장품허가 가능성 진단";
...
type SafetyDataStatus = "ready" | "not_ready" | null;
type DistributionChoice = "not_launched" | "distributing_licensed" | "distributing_unlicensed" | null;
...
const DISTRIBUTION_OPTIONS: { key: NonNullable<DistributionChoice>; label: string; desc: string; icon: typeof Store; tone: SelectionCardTone }[] = [
  { key: "not_launched", label: "아직 출시 전입니다", desc: "화장품 유통고시 신청을 준비하는 단계입니다.", icon: Store, tone: "blue" },
  { key: "distributing_licensed", label: "허가를 받고 정상 유통 중입니다", desc: "필요한 유통고시를 완료하고 판매 중입니다.", icon: CheckCircle2, tone: "green" },
  { key: "distributing_unlicensed", label: "허가 없이 이미 유통 중입니다", desc: "유통고시를 완료하지 않은 상태에서 판매 중입니다.", icon: AlertTriangle, tone: "red" },
];
...
const REGISTRATION_OPTIONS: { key: NonNullable<RegistrationStatus>; label: string; desc: string; icon: typeof FileCheck2; tone: SelectionCardTone }[] = [
  { key: "confirmed", label: "준비되어 있음", desc: "수입·유통업자 사업자 또는 법인 등록 서류를 보유하고 있습니다.", icon: FileCheck2, tone: "green" },
  { key: "unconfirmed", label: "아직 미확정", desc: "수입·유통업자 등록 서류를 아직 준비 중입니다.", icon: FileWarning, tone: "amber" },
];

const FACILITY_OPTIONS: { key: NonNullable<FacilityStatus>; label: string; desc: string; icon: typeof Store; tone: SelectionCardTone }[] = [
  { key: "secured", label: "확보 완료", desc: "제조·유통·보관 시설 또는 창고를 확보했습니다.", icon: Store, tone: "green" },
  { key: "unsecured", label: "아직 미확보", desc: "제조·유통·보관 시설 또는 창고가 아직 확정되지 않았습니다.", icon: FileWarning, tone: "amber" },
];

const SAFETY_DATA_OPTIONS: { key: NonNullable<SafetyDataStatus>; label: string; desc: string; icon: typeof Flame; tone: SelectionCardTone }[] = [
  { key: "ready", label: "예, 준비했습니다", desc: "제품 성분 및 안전성 평가자료를 준비했습니다.", icon: CheckCircle2, tone: "green" },
  { key: "not_ready", label: "아직입니다", desc: "제품 성분 또는 안전성 평가자료 준비가 남아 있습니다.", icon: Flame, tone: "amber" },
];
...
function computeCosmeticsDiagnosis(
  ...
  const checklist = [
    ...
    { label: "제조·유통·보관 시설(창고) 확보", passed: facilityStatus === "secured" },
    { label: "제품 성분·안전성 평가자료 준비", passed: safetyDataStatus === "ready" },
  ];
...
// 확인하여 구조·className·순서를 옮겨왔다. 값만 REGISTER(cosmetics) 진단 결과로 채운다.

// TRC의 PremiumLeadCapture 안 원형 게이지와 100% 동일한 마크업 — 결과 미리보기(가입 전)용.
function CosmeticsScoreGauge({
  score,
  tone,
}: {
  score: number;
...
// 추가하지 않는다.
function CosmeticsResultOverviewCards({
  diagnosis,
  docCount,
}: {
  diagnosis: CosmeticsDiagnosis;
  ...
  const items = [
    {
      n: 1,
      label: "허가 가능성",
      visual: <CosmeticsScoreGauge score={feasibilityScore} tone={resultTone} />,
...
function CosmeticsDesktopResultHeader({
  diagnosis,
}: {
  diagnosis: CosmeticsDiagnosis;
}) {
...
function CosmeticsResultSummaryCard({ diagnosis }: { diagnosis: CosmeticsDiagnosis }) {
...
// 길이·모바일/PC 배열을 그대로 일치시킨 3버튼 CTA. 서비스명(화장품허가)과 연결
// 대상(공식 사이트 URL, cosmetics CRM)만 Cosmetics에 맞게 대체했다.
...
function CosmeticsNextStepOptions({
  onSelf,
  onExpert,
  onAiReport,
  officialUrl,
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-4 text-lg font-bold text-gray-900">
              {isPossible ? "화장품허가 진행이 가능합니다" : "보완이 필요할 수 있습니다"}
            </p>

            <p className="mt-2 text-sm leading-relaxed text-gray-600">
              {isPossible
                ? "현재 사업자등록·시설 준비 상태 기준으로 화장품 유통고시(공고) 신청 요건을 충족합니다."
                : "현재 사업자등록 또는 시설 준비 상태만으로는 화장품허가 신청이 자동으로 진행되지 않습니다. 준비 서류를 보완하면 진행할 수 있는 경우가 많습니다."}
            </p>
          </div>

          <CosmeticsScoreGauge score={diagnosis.feasibilityScore} tone={diagnosis.resultTone} />
        </div>
...
export default function RegisterCosmeticsPage() {
  const [distributionChoice, setDistributionChoice] = useState<DistributionChoice>(null);
  ...
  const [safetyDataStatus, setSafetyDataStatus] = useState<SafetyDataStatus>(null);
  ...
  const isUnlicensedDistributing = distributionChoice === "distributing_unlicensed";
  ...
  const requiredDocs = getRequiredDocuments("register_cosmetics");
  ...
  async function handleExpertRequest() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=register_cosmetics&mode=expert`;
        return;
      }
  ...
  async function handleAiReportRequest() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=register_cosmetics&mode=ai_report`;
        return;
      }
  ...
  async function handleLeadSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    await supabase.from("crm_activities").insert({
      lead_id: newLeadId,
      action: "register_cosmetics_diagnosis_lead",
      tag: "REGISTER_COSMETICS",
      meta: diagnosis
        ? {
            feasibilityScore: diagnosis.feasibilityScore,
    ...
    >
      <FunnelPageHeader
        ...
        description={
          !costEntryDone
            ? contextTab === "review"
              ? "현재 받은 안내나 견적이 화장품허가에 필요한 절차와 비용 기준에 맞는지 확인합니다."
              : contextTab === "direct"
                ? "화장품허가의 기본 절차·준비 항목·추가 절차 가능성을 확인합니다."
      ...
      />
            ...
            <VerifyStepLayout
              ...
              question={
                <>
                  <QuestionSection
                    step={1}
                    title="이전에 다른 곳(정부기관 또는 타 대행사)에서 화장품허가(유통고시)를 신청하셨다가 거절·반려되신 적이 있나요?"
            ...
            {previousRejection === true && (
              <div className="mt-4">
                ...
                <textarea
                  ...
                  placeholder={
                    "예)\n- 수입·유통업자 등록 서류가 미비하다고 들었습니다.\n- 제품 성분·안전성 자료가 부족하다고 들었습니다.\n- 정확한 이유를 듣지 못했습니다.\n\n자유롭게 작성해주세요(선택)."
                  }
                  rows={6}
        ...
        {!restoreRegisterPending && costEntryDone && rejectionStepDone && !restoredLeadActive && isUnlicensedDistributing && (
          <div className={`mt-8 ${FUNNEL_QUESTION_COLUMN}`}>
            ...
            <div className="mt-5 flex flex-col gap-3">
              ...
              <Link
                href="/consultation?case=register-cosmetics-unlicensed-warning"
          ...
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              ...
              question={
                <QuestionSection
                  step={3}
                  title="사업자·법인(수입/유통업자) 등록 서류가 준비되어 있나요?"
                  description="화장품허가 신청에 앞서 확인하는 기본 등록 서류입니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
          ...
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              ...
              question={
                <QuestionSection
                  step={4}
                  title="제조·유통·보관 시설(창고)을 확보하셨나요?"
                  description="허가 신청 시 확인할 시설 확보 상태입니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
          ...
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              ...
              question={
                <QuestionSection
                  step={5}
                  title="제품 성분·안전성 평가자료를 준비하셨나요?"
                  description="화장품허가 관련 성분·안전성 자료 준비 상태를 확인합니다."
                  {...registerQuestionProps}
                >
                  <div className="grid grid-cols-2 gap-3">
                    {SAFETY_DATA_OPTIONS.map((opt) => (
                      <SelectionCard
                        key={opt.key}
                        variant="quiet"
                        title={opt.label}
        ...
        {!restoreRegisterPending && costEntryDone && canShowResults && diagnosis && leadSubmitted && (
          <div className="mt-8 rounded-3xl bg-white border border-gray-100 p-5 sm:p-6 shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
            ...
            <CosmeticsDesktopResultHeader diagnosis={diagnosis} />

            <CosmeticsResultOverviewCards diagnosis={diagnosis} docCount={requiredDocs.documents.length} />

            <CosmeticsResultSummaryCard diagnosis={diagnosis} />
            ...
            <p className="mt-2 text-[11px] text-gray-400">
              공공서비스포털(Cổng Dịch vụ công quốc gia)의 화장품허가 절차 안내
