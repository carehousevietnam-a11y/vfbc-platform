...
const CHECK_QUESTION_CONTEXT = "운전면허 전환 가능성 진단";
...
// 준비서류 안내/예상 처리기간/AI 검토 의견). 값은 전적으로 기존 진단 데이터
...
function buildResultSummaryText(
  ...
  const sentence1 = `입력하신 정보를 기준으로 운전면허 전환 가능성은 ${toneText} 것으로 확인되었습니다.`;
    ...
    <div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3 sm:items-stretch">
        ...
        <div className="relative flex h-full min-w-0 flex-col rounded-2xl border border-gray-100 bg-[#FAFBFC] px-3 py-3 sm:px-3.5 sm:py-3.5">
          ...
          <p className="mt-1.5 flex-1 text-xs leading-snug text-gray-500">
            정부 공식 사이트(운전면허 통합 포털)에서 운전면허 전환 신청을 직접 진행합니다.
          </p>
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <p className="mt-3 break-keep text-[16px] font-bold leading-snug text-gray-900 sm:text-[17px]">
              {isPossible ? "운전면허 전환이 가능합니다" : "보완이 필요할 수 있습니다"}
            </p>

            <p className="mt-2 break-keep text-[13px] leading-[1.55] text-[#556070] [overflow-wrap:normal]">
              {isPossible
                ? "거주증과 본국 면허를 보유하고 있어 베트남 운전면허로 전환 신청이 가능합니다."
                : "현재 조건만으로는 운전면허 전환이 자동으로 보장되지 않습니다. 추가 서류로 요건을 충족시킬 수 있는 경우가 많습니다."}
            </p>
          </div>
        ...
        <p className="mt-3 break-keep text-[11px] leading-[1.55] text-[#94A3B8] [overflow-wrap:normal]">
          * 위 결과는 입력하신 조건을 기준으로 한 1차 확인 결과입니다. 정확한
          전환 가능 여부는 서류 검토 후 전문가 상담을 통해 확정됩니다.
        </p>
...
export default function DrivingLicenseCheckPage() {
  ...
  async function handleLandingContinue() {
    const { loggedIn, restored } = await loadCheckMemberEntryState(
      "driving-license",
      "driving_license_diagnosis_lead",
      { allowRestore: true }
    );
  ...
  // 1) 로그인 회원 → 회원가입만 생략
  // 2) ?restore=1 → mount 시 기존 Driving License Case 복원 (?start=check 는 랜딩 스킵만, 복원 안 함)
  ...
  }, [trc, license, showResult, restoredLeadActive]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const created = await submitMemberCheckLead({
        ...
        sourcePage: "/check/driving-license",
        result,
        diagnosisAction: "driving_license_diagnosis_lead",
        tag: "DRIVING_LICENSE",
        diagnosis,
        previousRejection,
        rejectionReason,
  ...
  async function handleExpertRequestClick() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=driving-license&mode=expert`;
        return;
      }
  ...
  async function handleAiReportRequest() {
    ...
    try {
      ...
      if (hasSession) {
        window.location.href = `/documents?leadId=${encodeURIComponent(leadId)}&service=driving-license&mode=ai_report`;
        return;
      }
  ...
  return (
    ...
    >
      ...
      />
        ...
        {!restoreCheckPending && costEntryDone && rejectionStepDone && !trc && !restoredLeadActive && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="check"
              step={2}
              question={
                <QuestionSection
                  step={2}
                  title="현재 거주증(TRC)을 보유하고 계신가요?"
                  description="현재 상황에 맞춰 확인하기 위해 필요한 항목입니다. 거주증 보유 여부는 전환 가능 여부를 가르는 핵심 조건입니다."
                  {...checkQuestionProps}
                >
        ...
        {!restoreCheckPending && costEntryDone && rejectionStepDone && trc === "yes" && !license && !restoredLeadActive && (
          <div className="mt-4 sm:mt-5">
            <VerifyStepLayout
              engine="check"
              step={3}
              question={
                <QuestionSection
                  step={3}
                  title="본국(자국)에서 발급된 운전면허를 보유하고 계신가요?"
                  description="현재 상황에 맞춰 확인하기 위해 필요한 항목입니다. 본국 면허 소지 여부는 전환·신규 취득을 가르는 조건입니다."
                  {...checkQuestionProps}
                >
                  <VerifyAnswerGrid step={2}>
                    <SelectionCard
