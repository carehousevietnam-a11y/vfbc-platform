...
export function GuideCaseBody({ article }: GuideCaseBodyProps) {
  ...
  const costService = article.serviceType === "trc" ? getCostCheckService("trc") : null;

  const officialBasis = [...landing.officialBasis];
  if (costService && !officialBasis.some((line) => line.includes(costService.source))) {
    officialBasis.push(`정부 수수료 출처: ${costService.source}`);
  }
  ...
  if (costService && !sources.some((item) => item.label === costService.source)) {
    sources.push({
      ...
      detail: `정부 수수료 ${costService.governmentFee}`,
    });
  }
  ...
  return (
    <article className={`${ENGINE_CONTAINER} py-10 sm:py-14`}>
      ...
      <section className="mt-6 border-b border-slate-200/80 pb-6" aria-labelledby="guide-sources">
        <SectionHeading id="guide-sources">공식 출처</SectionHeading>
        <ul className="mt-3 space-y-2">
          {sources.map((item) => (
