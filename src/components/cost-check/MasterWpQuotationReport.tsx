...
import { WP_CHECK_MARKET_CASES } from "@/lib/contentPacks/checkMarketCases";
...
/** WP 확인하기 Master UI — 견적서 문서형. 다른 서비스에서는 사용하지 않음. */
const WP_SERVICE = getCostCheckService("wp");

/** Mobile — 업계 최저/최고 근처 공개 사례 각 1건 (전체 목록은 PC CheckMarketCaseRefs 유지). */
const WP_MOBILE_MARKET_CASE_PICKS = [
  WP_CHECK_MARKET_CASES[0],
  WP_CHECK_MARKET_CASES[WP_CHECK_MARKET_CASES.length - 1],
] as const;
...
/** 관공서 수수료 긴 문구 — 표 셀에서 옆 칸 침범·어색한 줄바꿈 방지 (문구 자체는 변경 없음) */
function WpGovernmentFeeLines({ fee }: { fee: string }) {
  const noteMatch = fee.match(/^(.*?)\s*(\([^)]+\))\s*$/);
  const main = noteMatch ? noteMatch[1] : fee;
  const note = noteMatch ? noteMatch[2] : null;

  return (
    <span className="block min-w-0 break-keep text-left sm:text-right">
      <span className="block text-[11px] font-bold tabular-nums leading-snug text-[#0B2A6B] sm:text-[11.5px]">
        {main}
      </span>
      {note ? (
        <span className="mt-0.5 block text-[10px] font-normal leading-snug text-[#64748B]">
          {note}
        </span>
      ) : null}
    </span>
...
const WP_ADDITIONAL_COST_ITEMS = [
...
/** WP 페이지·비용 안내에서 쓰는 현실적 위험 표현 (확정 벌금/법률 판단 아님) */
const WP_SELF_PROCEED_RISKS = [
  "학력·경력·직무 요건이나 서류 오류로 반려·보완·재제출이 생길 수 있습니다.",
  "신청 지연·서류 누락으로 일정이 늦어지고 추가 준비 비용이 생길 수 있습니다.",
] as const;

const WP_BAD_AGENCY_RISKS = [
  "불필요 항목·부실 대행으로 예상보다 높은 대행비·재신청 비용이 생길 수 있습니다.",
  "진행 지연·업체 변경으로 입사 일정 차질과 비용 중복이 생길 수 있습니다.",
] as const;

const WP_MARKET_WHY_BODY =
  "관공서 공식비용은 정부기관에 납부하는 수수료이고, 시장가격에는 서류 준비·번역·공증·행정 대응 등 대행 업무가 포함될 수 있습니다. 포함 항목을 확인하는 것이 중요합니다.";
...
export function MasterWpQuotationReport({
  onContinue,
  queryEntry,
}: {
  onContinue: () => void;
  /** 확인 시작용 질문 입력 — WP Master UI에서만 전달 */
  queryEntry?: ReactNode;
}) {
  const service = WP_SERVICE;
  ...
  /** WP 시장가격은 USD 기준 — VND 환산·신규 금액 생성 없음. 관공서 수수료는 기존 VND 문구 유지 */
  ...
  return (
    <div className="mt-2.5 space-y-2.5 sm:mt-3 sm:space-y-3">
      {queryEntry ? (
        <section
          aria-labelledby="wp-query-start"
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-[minmax(0,1fr)_270px] lg:items-stretch">
          <div className="flex min-w-0 flex-col border-[#E5E7EB] lg:border-r">
            ...
            <div className="flex-1 space-y-4 px-3.5 py-3 sm:space-y-4 sm:px-4 sm:py-3.5">
              <section aria-labelledby="wp-cost-summary">
                  ...
                  <table className="w-full table-fixed border-collapse border border-[#D8DEE8] text-[12px]">
                    ...
                    <tbody className="text-[#0F172A]">
                      <tr>
                        ...
                        <td className="border border-[#D8DEE8] px-1.5 py-1.5 align-middle sm:px-2 sm:py-2">
                          <WpGovernmentFeeLines fee={service.governmentFee} />
                        </td>
                ...
                {!hasQuote ? (
                  <div className="mt-2.5 border-t border-[#D8DEE8] pt-2 sm:mt-2.5">
                    <div className="flex items-baseline justify-between gap-3">
                      ...
                      <p
                        className={`min-w-0 text-right sm:text-[15px] sm:font-semibold sm:text-[#0B2A6B] ${MOBILE_PRICE_RANGE}`}
                      >
                        {marketUsdRange}
                      </p>
                    </div>
                    <div className="mt-1.5 sm:hidden">
                      <p className="text-[13px] font-medium leading-snug text-[#94A3B8]">
                        시장 사례 참고
                      </p>
                      <ul className="mt-1 space-y-0.5">
                        {WP_MOBILE_MARKET_CASE_PICKS.map((item) => (
                          <li
                            key={item.name}
              ...
              {hasQuote && grade ? (
                <section aria-labelledby="wp-cost-analysis">
                  <SectionTitle id="wp-cost-analysis" number="2" title="비용 분석 결과" emphasized />
              ...
              <section aria-labelledby="wp-extra-costs">
                <SectionTitle id="wp-extra-costs" number="3" title="추가 발생 가능 비용" emphasized />
                <p className={MOBILE_SECTION_INTRO}>
                  공식 수수료 외에 아래 비용이 추가로 발생할 수 있습니다.
                </p>
                <ul className="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-1.5 lg:grid-cols-5">
                  {WP_ADDITIONAL_COST_ITEMS.map((item) => (
                    <li
                      key={item.label}
              ...
              <section aria-labelledby="wp-risk-compare">
                <SectionTitle
                  id="wp-risk-compare"
                  number="4"
                ...
                <div className="grid grid-cols-1 items-start gap-2 lg:grid-cols-2 lg:gap-2">
                  <div className="h-auto self-start rounded-[6px] border border-[#FECACA] bg-[#FEF2F2]/60 px-3 py-2.5 sm:px-3 sm:py-2.5">
                    ...
                    <ul className="space-y-1">
                      {WP_SELF_PROCEED_RISKS.map((text, index) => (
                        <li
                          key={text}
                  ...
                  <div className="h-auto self-start rounded-[6px] border border-[#BFDBFE] bg-[#EFF6FF]/70 px-3 py-2.5 sm:px-3 sm:py-2.5">
                    ...
                    <ul className="space-y-1">
                      {WP_BAD_AGENCY_RISKS.map((text, index) => (
                        <li
                          key={text}
                          className={`flex gap-1.5 ${MOBILE_BODY}`}
                        >
                ...
                <p className={`mt-1.5 ${MOBILE_META} sm:mt-1.5`}>
                  확정된 법률·벌금 금액이 아니라, 노동허가 진행에서 확인하는 참고 안내입니다.
                </p>
              </section>

              <section aria-labelledby="wp-market-why">
                <SectionTitle
                  id="wp-market-why"
                  number="5"
            ...
            <footer className="mt-auto border-t border-[#E5E7EB] bg-[#F8FAFC] px-3.5 py-2.5 sm:px-4 sm:py-2">
              ...
              <p className="mt-1.5 break-keep text-center text-[10.5px] leading-relaxed text-[#64748B] sm:mt-1">
                노동허가 정부 수수료는 관할 지역에 따라 VND 금액이 달라집니다. 견적은 정부 수수료와
                대행·번역 비용이 함께 포함됐는지 확인하세요. 본 정보는 조사 시점 기준이며, 실제
                비용은 진행 시점에 반드시 재확인하시기 바랍니다.
              </p>
            </footer>
          </article>
        </div>
