...
const RESTAURANT_MODEL = getRegisterCostModel("restaurant");

const RESTAURANT_GOVERNMENT_LINKS = [
  {
    name: "국가공공서비스포털",
    url: "https://dichvucong.gov.vn/",
    detail: "식당·식품안전 관련 안내는 국가공공서비스포털에서 확인할 수 있습니다.",
  },
] as const;
...
/** REGISTER restaurant 비용견적 전용 사이드바 — CompanySummarySidebar와 동일 시각 구조. */
export function RestaurantSummarySidebar({
  quotedAmount,
  quoteCurrency = "VND",
  excessLabel,
  onContinue,
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <SidebarCard title="한눈에 보기">
          <div className="divide-y divide-[#EEF2F7]">
            <GlanceRow icon={ClipboardList} label="절차" value="식당허가 (ATTP)" />
            <GlanceRow
              icon={DollarSign}
              label="받은 견적"
              value={
        ...
        <SidebarCard title="관련 정부 기관">
          <ul className="space-y-2.5">
            {RESTAURANT_GOVERNMENT_LINKS.map((org) => (
