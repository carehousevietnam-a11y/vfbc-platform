...
import { DRIVING_CHECK_MARKET_CASES } from "@/lib/contentPacks/checkMarketCases";
...
const DRIVING_MOBILE_MARKET_CASE_PICKS = [
  DRIVING_CHECK_MARKET_CASES[0],
  DRIVING_CHECK_MARKET_CASES[1],
] as const;
...
/** 운전면허 확인하기 Master UI — 견적서 문서형. 다른 서비스에서는 사용하지 않음. */
const DRIVING_SERVICE = getCostCheckService("driving-license");

/** 기존 costCheck / evaluateCostQuoteReview 문구에 등장하는 항목만 (임의 금액 없음) */
const DRIVING_ADDITIONAL_COST_ITEMS = [
  { label: "서류 번역", note: "견적 포함 여부 확인" },
  { label: "공증", note: "견적 포함 여부 확인" },
  { label: "대행", note: "범위에 따라 상이" },
  { label: "서류 보완·재제출", note: "누락·반려 시 발생 가능" },
  { label: "방문 동행·기타", note: "범위에 따라 상이" },
] as const;

/** 운전면허 페이지·비용 안내에서 쓰는 현실적 위험 표현 (확정 벌금/법률 판단 아님) */
const DRIVING_SELF_PROCEED_RISKS = [
  "거주증(TRC)·본국 면허 요건이나 서류 오류로 교환 신청이 반려되거나 보완·재제출이 필요할 수 있습니다.",
  "신청 지연이나 서류 누락으로 일정이 늦어지고 추가 준비 비용이 발생할 수 있습니다.",
] as const;

const DRIVING_BAD_AGENCY_RISKS = [
  "불필요한 항목이나 부실한 대행으로 예상보다 높은 대행비와 재신청 비용이 발생할 수 있습니다.",
  "진행 지연이나 업체 변경으로 교환 일정에 차질이 생기고 비용이 중복될 수 있습니다.",
] as const;
...
export function MasterDrivingQuotationReport({
  onContinue,
  queryEntry,
}: {
  onContinue: () => void;
  /** 확인 시작용 질문 입력 — 운전면허 Master UI에서만 전달 */
  queryEntry?: ReactNode;
}) {
  const service = DRIVING_SERVICE;
  ...
  /** 운전면허 관공서 수수료는 기존 VND 문구 유지. 시장가격 데이터 없으면 신규 안내 문구만 사용(금액 신규 생성 없음) */
  const hasMarket =
    service.marketMin > 0 || service.marketMax > 0 || service.marketUsualFeeAmount > 0;
  const marketDisplay = hasMarket
    ? `${formatCostAmount(service.marketMin, service.currency)} ~ ${formatCostAmount(
        service.marketMax,
        service.currency
      )}`
    : "시장가격 확인 중";
  const marketHint = hasMarket
    ? "일반적인 시장가격 범위"
    : "번역·공증 등 추가 비용은 별도로 발생할 수 있습니다";
  const quotePlaceholder = `예: ${service.govFeeAmount.toLocaleString("ko-KR")}`;
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-[minmax(0,1fr)_270px] lg:items-stretch">
          <div className="flex min-w-0 flex-col border-[#E5E7EB] lg:border-r">
              ...
              <div className="flex flex-col gap-1.5 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 break-words text-[12px] font-normal leading-snug tracking-[0.02em] text-[#64748B] sm:mt-0.5 sm:text-[10px] sm:leading-relaxed sm:tracking-[0.06em]">
                    QUOTATION REPORT · DRIVING LICENSE
                  </p>
                </div>
            ...
            <div className="flex-1 space-y-4 px-3.5 py-3 sm:space-y-4 sm:px-4 sm:py-3.5">
              <section aria-labelledby="driving-cost-summary">
                  ...
                  <table className="w-full table-fixed border-collapse border border-[#D8DEE8] text-[12px]">
                    ...
                    <tbody className="text-[#0F172A]">
                      <tr>
                        ...
                        <td className="border border-[#D8DEE8] px-1.5 py-1.5 text-right align-middle font-bold tabular-nums leading-snug text-[#0B2A6B] sm:px-2 sm:py-2">
                          {service.governmentFee}
                        </td>
                  ...
                  >
                    <label className="min-w-0 flex-1">
                      <span className="block text-[14px] font-normal leading-snug text-[#475569] sm:text-[11.5px] sm:leading-relaxed">
                        받은 견적 입력 시 비교 ({service.currency})
                      </span>
                      <input
                        type="text"
                        inputMode="decimal"
                  ...
                  <div className="mt-2.5 border-t border-[#D8DEE8] pt-2 sm:mt-2.5">
                    ...
                    <div className="mt-1.5 sm:hidden">
                      ...
                      <ul className="mt-1 space-y-0.5">
                        {DRIVING_MOBILE_MARKET_CASE_PICKS.map((item) => (
                          <li
                            key={item.name}
                    ...
                    <CheckMarketCaseRefs
                      cases={DRIVING_CHECK_MARKET_CASES}
              ...
              {hasQuote && grade ? (
                <section aria-labelledby="driving-cost-analysis">
                  <SectionTitle id="driving-cost-analysis" number="2" title="비용 분석 결과" emphasized />
                  ...
                  >
                    <div className="flex flex-col gap-2.5 lg:flex-row lg:items-start lg:justify-between lg:gap-2.5">
                      ...
                      <div className="shrink-0 rounded-[6px] border border-white/80 bg-white/80 px-3 py-2 sm:px-3 sm:py-2 lg:min-w-[8.5rem]">
                        <p className="text-[12px] font-normal text-[#64748B] sm:text-[10px]">과다 가능성 등급</p>
              ...
              <section aria-labelledby="driving-extra-costs">
                <SectionTitle id="driving-extra-costs" number="3" title="추가 발생 가능 비용" emphasized />
                <p className={MOBILE_SECTION_INTRO}>
                  공식 수수료 외에 아래 비용이 추가로 발생할 수 있습니다.
                </p>
                <ul className="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-1.5 lg:grid-cols-5">
                  {DRIVING_ADDITIONAL_COST_ITEMS.map((item) => (
                    <li
                      key={item.label}
              ...
              <section aria-labelledby="driving-risk-compare">
                <SectionTitle
                  id="driving-risk-compare"
                  number="4"
                  title="이런 경우 추가 비용과 문제가 발생할 수 있습니다"
                  emphasized
                />
                <p className={MOBILE_SECTION_INTRO}>
                  직접 진행과 대행 선택에서 흔히 확인하는 참고 안내입니다.
                </p>
                <div className="grid grid-cols-1 items-start gap-2 lg:grid-cols-2 lg:gap-2">
                  <div className="h-auto self-start rounded-[6px] border border-[#FECACA] bg-[#FEF2F2]/60 px-3 py-2.5 sm:px-3 sm:py-2.5">
                    <div className="mb-1.5 flex items-start gap-2 sm:mb-1.5 sm:items-center sm:gap-1.5">
                    ...
                    <ul className="space-y-1">
                      {DRIVING_SELF_PROCEED_RISKS.map((text, index) => (
                        <li
                          key={text}
                  ...
                  <div className="h-auto self-start rounded-[6px] border border-[#BFDBFE] bg-[#EFF6FF]/70 px-3 py-2.5 sm:px-3 sm:py-2.5">
                    ...
                    <ul className="space-y-1">
                      {DRIVING_BAD_AGENCY_RISKS.map((text, index) => (
                        <li
                          key={text}
                          className={`flex gap-1.5 ${MOBILE_BODY}`}
                        >
                ...
                <p className={`mt-1.5 ${MOBILE_META} sm:mt-1.5`}>
                  확정된 법률·벌금 금액이 아니라, 운전면허 교환에서 확인하는 참고 안내입니다.
                </p>
              </section>

              <section aria-labelledby="driving-market-why">
                <SectionTitle
                  id="driving-market-why"
                  number="5"
                ...
                <div className="rounded-[6px] border border-[#BBF7D0] bg-[#F0FDF4] px-3 py-2.5 sm:px-3 sm:py-2.5">
                  <div className="flex items-start gap-2.5 sm:gap-2">
                    ...
                    <p className={MOBILE_CALLOUT}>
                      관공서 공식비용은 정부기관에 납부하는 수수료입니다. 시장가격에는 번역·공증 등
                      추가 비용이 포함될 수 있습니다. 따라서 가격 차이만으로 비싸다고 판단하기보다,
                      견적에 어떤 업무와 비용이 포함되어 있는지 확인하는 것이 중요합니다.
                    </p>
                  </div>
                </div>
              </section>
            ...
            <footer className="mt-auto border-t border-[#E5E7EB] bg-[#F8FAFC] px-3.5 py-2 sm:px-4 sm:py-2">
              ...
              <p className="mt-1 break-keep text-center text-[11px] leading-[1.45] text-[#64748B] sm:mt-1 sm:text-[10.5px] sm:leading-relaxed">
                외국인 운전면허 교환 정부 수수료는 국가공공서비스 포털 기준입니다. 번역·공증 등
                추가 비용은 별도로 발생할 수 있습니다. 본 정보는 조사 시점 기준이며, 실제 비용은
                진행 시점에 반드시 재확인하시기 바랍니다.
              </p>
            </footer>
          </article>
        </div>
