...
import {
  ...
  COMPANY_REGISTER_MARKET_CASES,
  getRegisterCostModel,
} from "@/lib/registerCostCheck";
import { COMPANY_QUOTATION_DETAIL } from "@/lib/registerQuotationDetail";
...
/**
 * REGISTER company 비용견적 — CHECK(WP/Driving)와 동일 섹션·표·분석 구조.
 * 분석은 evaluateCostQuoteReview 재사용(FDI USD 시장 기준).
 */
export function MasterCompanyQuotationReport({
  onContinue,
  queryEntry,
}: {
  onContinue: () => void;
  ...
  const model = getRegisterCostModel("company");
  const detail = COMPANY_QUOTATION_DETAIL;
  const reviewService = COMPANY_FDI_REVIEW_SERVICE;
  ...
  const marketUsdRange = `${formatCostAmount(reviewService.marketMin, "USD")} ~ ${formatCostAmount(
          ...
          <div className="flex min-w-0 flex-col border-[#E5E7EB] lg:border-r">
            <article className="flex min-h-0 flex-col">
              ...
              <div className="space-y-3 px-3.5 py-3 sm:space-y-4 sm:px-4 sm:py-3.5">
                ...
                <section aria-labelledby="company-risk-compare">
                  <SectionTitle
                    id="company-risk-compare"
                    number="4"
