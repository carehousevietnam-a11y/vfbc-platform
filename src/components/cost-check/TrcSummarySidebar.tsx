...
/**
 * TRC 공식 경로 — MasterFunnelLanding / check/trc 페이지에 이미 존재하는 URL·문구만 사용.
 * (임의 기관·URL 생성 금지)
 */
const TRC_GOVERNMENT_LINKS = [
  {
    name: "공안부 공공서비스포털",
    url: "https://dichvucong.bocongan.gov.vn/bocongan/bothutuc/tthc?matt=26285",
    detail: "공안부 공공서비스포털의 거주증(TRC) 발급 절차 안내를 확인할 수 있습니다.",
  },
] as const;

/** 기존 TRC 위험 문구에서 가져온 짧은 요약 (벌금·법률 확정 표현 없음) */
const TRC_RISK_SUMMARY = "재신청 · 지연 · 추가 비용";
...
/**
 * TRC 확인하기 Master UI 전용 오른쪽 요약 사이드바.
 * 다른 서비스에서는 사용하지 않음.
 * Mobile(<lg): 미표시 (CTA는 견적서 본문 녹색 카드 아래에 배치)
 * PC(lg+): 기존 요약 사이드바 + CTA 유지
 */
export function TrcSummarySidebar({
  quotedAmount,
  currency,
  excessLabel,
  onContinue,
  ...
  currency: "USD" | "VND";
  /** evaluateCostQuoteReview → excessGrade 라벨. 견적 없으면 null */
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <SidebarCard title="한눈에 보기">
          <div className="divide-y divide-[#EEF2F7]">
            <GlanceRow icon={ClipboardList} label="절차" value="거주증 (TRC)" />
            <GlanceRow
              icon={DollarSign}
              label="받은 견적"
              value={
            ...
            <GlanceRow
              icon={UserRound}
              label="주요 위험"
              value={TRC_RISK_SUMMARY}
              tone="risk"
            />
          </div>
        </SidebarCard>

        {TRC_GOVERNMENT_LINKS.length > 0 ? (
          <SidebarCard title="관련 정부 기관">
            <ul className="space-y-2.5">
              {TRC_GOVERNMENT_LINKS.map((org) => (
        ...
        <SidebarCard title="다음 단계" className="flex min-h-0 flex-1 flex-col">
          ...
          <div className="mt-auto pt-3.5 sm:pt-3">
            ...
            {/* 공유 시스템은 프로젝트에 없음 — UI만 표시, 기능 임의 구현 금지 */}
            <button
              type="button"
