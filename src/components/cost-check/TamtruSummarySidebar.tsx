...
/**
 * Tamtru 공식 경로 — MASTER_LANDING_TAMTRU에 이미 존재하는 URL·문구만 사용.
 * (임의 기관·URL 생성 금지)
 */
const TAMTRU_GOVERNMENT_LINKS = [
  {
    name: "evisa 포털",
    url: "https://evisa.gov.vn/khai-bao-tam-tru",
    detail: "임시거주 신고 관련 공식 안내는 evisa 포털에서 확인할 수 있습니다.",
  },
] as const;

/** 기존 Tamtru 위험 문구에서 가져온 짧은 요약 (벌금·법률 확정 표현 없음) */
const TAMTRU_RISK_SUMMARY = "반려 · 지연 · 추가 비용";
...
/**
 * Tamtru 확인하기 Master UI 전용 오른쪽 요약 사이드바.
 * 다른 서비스에서는 사용하지 않음.
 * Mobile(<lg): 미표시 (CTA는 견적서 본문 녹색 카드 아래에 배치)
 * PC(lg+): 기존 요약 사이드바 + CTA 유지
 */
export function TamtruSummarySidebar({
  quotedAmount,
  currency,
  excessLabel,
  onContinue,
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        <SidebarCard title="한눈에 보기">
          <div className="divide-y divide-[#EEF2F7]">
            <GlanceRow icon={ClipboardList} label="절차" value="임시거주 (땀주)" />
            <GlanceRow
              icon={DollarSign}
              label="받은 견적"
              value={
            ...
            <GlanceRow
              icon={UserRound}
              label="주요 위험"
              value={TAMTRU_RISK_SUMMARY}
              tone="risk"
            />
          </div>
        </SidebarCard>

        {TAMTRU_GOVERNMENT_LINKS.length > 0 ? (
          <SidebarCard title="관련 정부 기관">
            <ul className="space-y-2.5">
              {TAMTRU_GOVERNMENT_LINKS.map((org) => (
