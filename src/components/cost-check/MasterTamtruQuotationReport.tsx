...
import { TAMTRU_CHECK_MARKET_CASES } from "@/lib/contentPacks/checkMarketCases";
...
/** Tamtru 확인하기 Master UI — 견적서 문서형. 다른 서비스에서는 사용하지 않음. */
const TAMTRU_SERVICE = getCostCheckService("tamtru");

/** Mobile — 공개 사례 1건 (전체 목록은 PC CheckMarketCaseRefs 유지). */
const TAMTRU_MOBILE_MARKET_CASE_PICKS = [TAMTRU_CHECK_MARKET_CASES[0]] as const;
...
const TAMTRU_ADDITIONAL_COST_ITEMS = [
  { label: "서류 번역", note: "견적 포함 여부 확인" },
  { label: "이동", note: "견적 포함 여부 확인" },
  { label: "대행", note: "범위에 따라 상이" },
  { label: "서류 준비", note: "견적 포함 여부 확인" },
  { label: "서류 보완·재제출", note: "누락·반려 시 발생 가능" },
] as const;

/** Tamtru 페이지·비용 안내에서 쓰는 현실적 위험 표현 (확정 벌금/법률 판단 아님) */
const TAMTRU_SELF_PROCEED_RISKS = [
  "숙소 형태·집주인 협조나 등록 기한 문제로 신고가 반려되거나 보완·재등록이 필요할 수 있습니다.",
  "신청 지연이나 서류 누락으로 일정이 늦어지고 추가 준비 비용이 발생할 수 있습니다.",
] as const;

const TAMTRU_BAD_AGENCY_RISKS = [
  "불필요한 항목이나 부실한 대행으로 예상보다 높은 대행비와 재신청 비용이 발생할 수 있습니다.",
  "진행 지연이나 업체 변경으로 체류·숙소 일정에 차질이 생기고 비용이 중복될 수 있습니다.",
] as const;
...
export function MasterTamtruQuotationReport({
  onContinue,
  queryEntry,
}: {
  onContinue: () => void;
  /** 확인 시작용 질문 입력 — Tamtru Master UI에서만 전달 */
  queryEntry?: ReactNode;
}) {
  const service = TAMTRU_SERVICE;
  ...
  /** Tamtru 시장가격은 USD 기준 — VND 환산·신규 금액 생성 없음. 관공서 수수료는 기존 \"무료\" 문구 유지 */
          ...
          <div className="flex min-w-0 flex-col border-[#E5E7EB] lg:border-r">
            <article className="flex min-h-0 flex-col">
                ...
                <div className="flex flex-col gap-1.5 sm:flex-row sm:items-start sm:justify-between sm:gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-0.5 break-words text-[12px] font-normal leading-snug tracking-[0.02em] text-[#64748B] sm:mt-0.5 sm:text-[10px] sm:leading-relaxed sm:tracking-[0.06em]">
                      QUOTATION REPORT · TEMPORARY RESIDENCE
                    </p>
                  </div>
              ...
              <div className="space-y-4 px-3.5 py-3 sm:space-y-4 sm:px-4 sm:py-3.5">
                <section aria-labelledby="tamtru-cost-summary">
                    ...
                    <div className="mt-2.5 border-t border-[#D8DEE8] pt-2 sm:mt-2.5">
                      ...
                      <div className="mt-1.5 sm:hidden">
                        ...
                        <ul className="mt-1 space-y-0.5">
                          {TAMTRU_MOBILE_MARKET_CASE_PICKS.map((item) => (
                            <li
                              key={item.name}
                      ...
                      <CheckMarketCaseRefs
                        cases={TAMTRU_CHECK_MARKET_CASES}
                ...
                {hasQuote && grade ? (
                  <section aria-labelledby="tamtru-cost-analysis">
                    <SectionTitle id="tamtru-cost-analysis" number="2" title="비용 분석 결과" emphasized />
                ...
                <section aria-labelledby="tamtru-extra-costs">
                  <SectionTitle id="tamtru-extra-costs" number="3" title="추가 발생 가능 비용" emphasized />
                  <p className={MOBILE_SECTION_INTRO}>
                    공식 수수료 외에 아래 비용이 추가로 발생할 수 있습니다.
                  </p>
                  <ul className="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-1.5 lg:grid-cols-5">
                    {TAMTRU_ADDITIONAL_COST_ITEMS.map((item) => (
                      <li
                        key={item.label}
                ...
                <section aria-labelledby="tamtru-risk-compare">
                  <SectionTitle
                    id="tamtru-risk-compare"
                    number="4"
                  ...
                  <div className="grid grid-cols-1 items-start gap-2 lg:grid-cols-2 lg:gap-2">
                    <div className="h-auto self-start rounded-[6px] border border-[#FECACA] bg-[#FEF2F2]/60 px-3 py-2.5 sm:px-3 sm:py-2.5">
                      ...
                      <ul className="space-y-1">
                        {TAMTRU_SELF_PROCEED_RISKS.map((text, index) => (
                          <li
                            key={text}
                    ...
                    <div className="h-auto self-start rounded-[6px] border border-[#BFDBFE] bg-[#EFF6FF]/70 px-3 py-2.5 sm:px-3 sm:py-2.5">
                      ...
                      <ul className="space-y-1">
                        {TAMTRU_BAD_AGENCY_RISKS.map((text, index) => (
                          <li
                            key={text}
                            className={`flex gap-1.5 ${MOBILE_BODY}`}
                          >
                  ...
                  <p className={`mt-1.5 ${MOBILE_META} sm:mt-1.5`}>
                    확정된 법률·벌금 금액이 아니라, 임시거주 신고에서 확인하는 참고 안내입니다.
                  </p>
                </section>

                <section aria-labelledby="tamtru-market-why">
                  <SectionTitle
                    id="tamtru-market-why"
                    number="5"
                  ...
                  <div className="rounded-[6px] border border-[#BBF7D0] bg-[#F0FDF4] px-3 py-2.5 sm:px-3 sm:py-2.5">
                    <div className="flex items-start gap-2.5 sm:gap-2">
                      ...
                      <p className={MOBILE_CALLOUT}>
                        임시거주 신고 자체는 정부 수수료가 없습니다. 시장가격에는 대행·이동·서류 준비
                        등 실무 비용이 포함될 수 있습니다. 따라서 가격 차이만으로 비싸다고
                        판단하기보다, 견적에 어떤 업무와 비용이 포함되어 있는지 확인하는 것이
                        중요합니다.
                      </p>
                    </div>
                  </div>
              ...
              <footer className="border-t border-[#E5E7EB] bg-[#F8FAFC] px-3.5 py-2 sm:px-4 sm:py-2">
                ...
                <p className="mt-1 break-keep text-center text-[11px] leading-[1.45] text-[#64748B] sm:mt-1 sm:text-[10.5px] sm:leading-relaxed">
                  임시거주 신고 자체는 정부 수수료가 없지만, 대행·이동·서류 준비 비용은 별도로 발생할 수
                  있습니다. 본 정보는 조사 시점 기준이며, 실제 비용은 진행 시점에 반드시
                  재확인하시기 바랍니다.
                </p>
              </footer>
            </article>
