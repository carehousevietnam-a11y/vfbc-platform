...
import { RESTAURANT_QUOTATION_DETAIL } from "@/lib/registerQuotationDetail";
...
/**
 * REGISTER restaurant 비용견적 — Company Master와 동일 섹션·표·분석 구조.
 * 분석은 evaluateCostQuoteReview 재사용(식당 ATTP VND 시장 기준).
 */
export function MasterRestaurantQuotationReport({
  onContinue,
  queryEntry,
}: {
  onContinue: () => void;
  ...
  const model = getRegisterCostModel("restaurant");
  const detail = RESTAURANT_QUOTATION_DETAIL;
  const reviewService = RESTAURANT_REVIEW_SERVICE;
  ...
  }, [submittedAmount, reviewService]);

  if (!model) return null;

  const marketVndRange = `${formatCostAmount(reviewService.marketMin, "VND")} ~ ${formatCostAmount(
          ...
          <div className="flex min-w-0 flex-col border-[#E5E7EB] lg:border-r">
            <article className="flex min-h-0 flex-col">
              ...
              <div className="space-y-4 px-3.5 py-3.5 sm:px-4 sm:py-3.5">
                <section aria-labelledby="restaurant-cost-summary">
                    ...
                    <table className="w-full table-fixed border-collapse border border-[#D8DEE8] text-[12px]">
                      ...
                      <tbody className="text-[#0F172A]">
                        <tr>
                          ...
                          <td className="border border-[#D8DEE8] px-1.5 py-1.5 text-right align-middle font-bold leading-snug text-[#0B2A6B] sm:px-2 sm:py-2">
                            {model.governmentSummary}
                          </td>
                  ...
                  {!hasQuote ? (
                    <div className="mt-3 border-t border-[#D8DEE8] pt-2.5 sm:mt-2.5">
                      ...
                      <CheckMarketCaseRefs cases={RESTAURANT_REGISTER_MARKET_CASES} className="mt-1.5" />
                    </div>
                  ) : null}
                </section>
                ...
                <section aria-labelledby="restaurant-risk-compare">
                  <SectionTitle
                    id="restaurant-risk-compare"
                    number="4"
                ...
                <section aria-labelledby="restaurant-market-why">
                  <SectionTitle id="restaurant-market-why" number="5" title={detail.marketWhyTitle} />
