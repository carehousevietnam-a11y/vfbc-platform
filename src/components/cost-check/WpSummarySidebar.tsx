...
/**
 * WP 공식 경로 — MasterFunnelLanding MASTER_LANDING_WP에 이미 존재하는 URL·문구만 사용.
 * (임의 기관·URL 생성 금지)
 */
const WP_GOVERNMENT_LINKS = [
  {
    name: "국가공공서비스포털",
    url: "https://dichvucong.gov.vn/",
    detail: "외국인노동자 관리 관련 공공서비스 안내는 국가공공서비스포털에서 확인할 수 있습니다.",
  },
] as const;

/** WP 기존 안내·진단 맥락의 짧은 요약 (벌금·법률 확정 표현 없음) */
const WP_RISK_SUMMARY = "반려 · 지연 · 추가 비용";
...
/**
 * WP 확인하기 Master UI 전용 오른쪽 요약 사이드바.
 * 다른 서비스에서는 사용하지 않음.
 * Mobile(<lg): 미표시 (CTA는 견적서 본문 녹색 카드 아래에 배치)
 * PC(lg+): 기존 요약 사이드바 + CTA 유지
 */
export function WpSummarySidebar({
  quotedAmount,
  currency,
  excessLabel,
  onContinue,
    ...
    <aside className="hidden h-full w-full flex-col border-t border-[#E5E7EB] bg-[#F8FAFC] p-3.5 lg:flex lg:border-t-0 lg:w-auto lg:p-3">
      <SidebarCard title="한눈에 보기">
        <div className="divide-y divide-[#EEF2F7]">
          <GlanceRow icon={ClipboardList} label="절차" value="노동허가증 (WP)" />
          <GlanceRow
            icon={DollarSign}
            label="받은 견적"
            value={
          ...
          <GlanceRow
            icon={UserRound}
            label="주요 위험"
            value={WP_RISK_SUMMARY}
            tone="risk"
          />
        </div>
      </SidebarCard>

      {WP_GOVERNMENT_LINKS.length > 0 ? (
        <SidebarCard title="관련 정부 기관">
          <ul className="space-y-2.5">
            {WP_GOVERNMENT_LINKS.map((org) => (
              <li key={org.url} className="min-w-0">
                <p className="text-[12.5px] font-medium leading-snug text-[#0B2A6B] sm:text-[12px]">{org.name}</p>
                {org.detail ? (
            ...
            return (
              <li key={step.title} className="relative flex gap-2.5 pb-3 last:pb-0 sm:gap-2 sm:pb-2.5">
                ...
                <div className="min-w-0 pt-0.5">
                  <p className="text-[12px] font-medium leading-snug text-[#0B2A6B] sm:text-[11.5px]">{step.title}</p>
